...
AtPRAtask::AtPRAtask() : FairTask("AtPRAtask")
{
   ...
   gLogger->Debug(MESSAGE_ORIGIN, "Default Constructor of AtPRAtask");
   fPar = NULL;
   fPRAlgorithm = 0;
...
AtPRAtask::~AtPRAtask()
{
   gLogger->Debug(MESSAGE_ORIGIN, "Destructor of AtPRAtask");
}
...
{
   gLogger->Debug(MESSAGE_ORIGIN, "SetParContainers of AtPRAtask");
...
{
   gLogger->Debug(MESSAGE_ORIGIN, "Initilization of AtPRAtask");

   fPatternEventArray = new TClonesArray("AtPatternEvent");

   if (fPRAlgorithm == 0) {
      gLogger->Info(MESSAGE_ORIGIN, "Using Track Finder Hierarchical Clustering algorithm");
      ...
      gLogger->Info(MESSAGE_ORIGIN, "Using RANSAC algorithm");

   } else if (fPRAlgorithm == 2) {
      gLogger->Info(MESSAGE_ORIGIN, "Using Hough transform algorithm");
   }
...
{
   gLogger->Debug(MESSAGE_ORIGIN, "Exec of AtPRAtask");
...
{
   gLogger->Debug(MESSAGE_ORIGIN, "Finish of AtPRAtask");
}
