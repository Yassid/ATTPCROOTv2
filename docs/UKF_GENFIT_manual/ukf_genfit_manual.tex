...
\begin{itemize}[nosep]
  ...
  (class \code{EventFit::AtGenfitter}, \code{KalmanFitterRefTrack}). It keeps only
  ...
  vertex). It derives from the \emph{modern} \code{EventFit::AtFitter} base, so it
...
\textbf{Macro:} \file{pipeline/fitGenfitter\_a1975.C}. Same input
...
path}. It uses the clean \code{EventFit::AtGenfitter} on the \emph{modern}
...
\toprule
 & \textbf{UKF}\newline(\code{AtFitterUKF}) & \textbf{AtGenfitter}\newline(clean) & \textbf{AtGenfit}\newline(legacy) \\
\midrule
Algorithm     & Unscented KF + RTS smoother & \code{KalmanFitterRefTrack} & \code{KalmanFitterRefTrack} \\
...
Field sign    & signed \code{SetBField}     & signed \code{SetBField} ($-2.85$ exp) & \code{SetSimulationConvention(0)} \\
Re-clustering & internal / adaptive         & none (drift-$z$ ordering) & \code{EnableReclustering(\dots)} \\
PID gating    & --- (multi-hyp $\chi^2$)    & \code{SetPIDGate} (internal Spyral PID) & --- \\
Task wrapper  & \code{AtFitterTask}         & \code{AtFitterTask}        & \code{AtFitterTaskOld} \\
Output        & \code{AtTrackingEvent} + \code{AtFitMetadata} & \code{AtTrackingEvent} + \code{AtFitMetadata} & \code{AtTrackingEvent} \\
Status        & native, recommended         & recommended genfit         & \code{[[deprecated]]} fallback \\
\bottomrule
\end{tabular}
\end{center}
...
\code{AtGenfit} instead uses \code{SetSimulationConvention(false)} for experimental
data.
