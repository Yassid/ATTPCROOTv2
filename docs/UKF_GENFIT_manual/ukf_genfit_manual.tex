...
% Long unbreakable typewriter tokens (class names, file paths) can protrude into
% the right margin. A small emergencystretch lets TeX, as a last resort, drop such
% a token to a fresh line (where it fits) instead of overflowing -- without
% loosening normal paragraphs. The worst multi-slash paths also carry explicit
% \allowbreak hints at their separators.
\tolerance=1500
...
using \textbf{ATTPCROOTv2-OpenKF}, covering the track fitters available in the
framework:
...
\begin{itemize}[nosep]
  ...
  \item \textbf{AtGenfitter} --- the \emph{clean} GENFIT2 reference-track fitter
  (class \code{EventFit::\allowbreak AtGenfitter}, \code{KalmanFitterRefTrack}). It keeps only
  the working genfit core (\code{RKTrackRep} + \code{AtSpacepointMeasurement}) and
  shares the validated UKF frame conventions (uniform $z_\mathrm{lab}=$
  \code{ZPadPlane}$-z_\mathrm{digi}$, signed $B$ field, highest-$z_\mathrm{digi}$
  vertex). It derives from the \emph{modern} \code{EventFit::\allowbreak AtFitter} base, so it
  runs in the maintained \code{AtFitterTask}, and it can gate which species it
  fits via an upstream Spyral-PID cut (\S\ref{sec:pidgate}).
  \item \textbf{AtGenfit} (legacy) --- the original GENFIT2 fitter
  (class \code{AtFITTER::AtGenfit}, run via \code{AtFitterTaskOld}), with SRIM-style
  energy-loss tables and built-in merging / reclustering / single-vertex pooling.
  It is \code{[[deprecated]]} but retained as a validated fallback.
\end{itemize}

All three fitters consume the \emph{same} pattern-recognition output
...
task, which makes it easy to fit identical track candidates with different methods
and compare.
...
its tuning has been validated. \code{AtGenfitter} is now the \textbf{recommended
genfit path}: it reaches genfit's analytic-Jacobian resolution while sharing the
UKF conventions and adding PID gating; on a1975 \code{run\_0106} it converges on
$\approx 95.6\%$ of tracks ($\approx 91\%$ forward) and runs $\sim 15\times$ faster
than the legacy \code{AtGenfit}. Reach for the legacy \code{AtGenfit} only to
reproduce older results. The fitters are best used \emph{together}: reconstruct
once, then fit with whichever combination you need and compare.
\end{notebox}
...
Section~\ref{sec:pidgate} covers particle identification and the in-fitter PID gate.
...
\begin{description}[leftmargin=1.4cm,style=nextline]
  ...
  branch \code{AtPatternEvent}. \code{AtPRAtask} now also accepts the
  finder algorithm by \emph{dependency injection} (a \code{unique\_ptr}, like
  \code{AtFitterTask}); see \S\ref{sec:prainject}.
  \item[Fitting] \code{AtFitterTask} (UKF, multi-hypothesis UKF, or the clean
  \code{AtGenfitter}) or the legacy \code{AtFitterTaskOld} (\code{AtGenfit}) fits
  each candidate, producing \code{AtTrackingEvent} (a collection of
  \code{AtFittedTrack}) and optionally \code{AtFitMetadata}. An \code{AtPIDTask}
  may run alongside the fitter to write \code{AtPIDEvent} (Spyral PID observables);
  \code{AtGenfitter} can also compute the same PID internally to gate its input.
\end{description}
...
\file{AtReconstruction/\allowbreak CMakeLists.txt} only compiles \file{AtGenfit.cxx} and
\file{AtSpacePoint\allowbreak Measurement.cxx} when \code{GENFIT2\_FOUND} is true:
...
\file{macro/\allowbreak Simulation/\allowbreak ATTPC/\allowbreak 16C\_pp/}. Run
...
(parameters), \file{data/\allowbreak geofile\_full.root} (geometry).
...
Parameter file \file{\$VMCWORKDIR/\allowbreak parameters/\allowbreak
ATTPC.e20009\_sim.par}, pad map \file{\$VMCWORKDIR/\allowbreak scripts/\allowbreak
Lookup20150611.xml}.
...
\begin{longtable}{@{}p{4.4cm} p{3.7cm} p{7.0cm}@{}}
\toprule
...
\begin{notebox}
\textbf{Genfit one-shot macros.} For a single-pass unpack$\rightarrow$genfit run
there are standalone macros one level up, in \file{macro/Unpack\_HDF5/a1975/}:
\file{unpackNFit\_a1975.C} fits the proton $(p,p)$ channel and
\file{unpackNFit\_a1975\_deuterium.C} the deuteron $(p,d)$ channel. Both run the
full \code{Unpack}$\rightarrow$\code{PSA}$\rightarrow$SC$\rightarrow$\code{PRA}
(injected) $\rightarrow$\code{EventFit::AtGenfitter} chain and take
\code{(TString fileName="run\_0106", Long64\_t nEvents=-1)}.
\end{notebox}

\code{unpackReco\_a1975\_UKF.C} is the \textbf{standard entry point}: it produces
the \code{AtPatternEvent} that all three fitters consume. Its task chain:
...
// PRA by dependency injection: the task takes a pre-configured finder
// (AtTrackFinderTC already carries the standard TC defaults), so only the
// cluster radius/distance are set here. See section 5.2.
...
\subsection{Pattern recognition by dependency injection}\label{sec:prainject}

\code{AtPRAtask} now mirrors \code{AtFitterTask}: it can \emph{own} a
pre-configured finder passed as a \code{unique\_ptr}, instead of constructing one
internally from an integer selector. The task then only forwards the runtime
\code{AtDigiPar} diffusion parameters and handles branch I/O.

\begin{lstlisting}
// modern (injected) form -- the task owns the algorithm
AtPRAtask(std::unique_ptr<AtPATTERN::AtPRA> pra);
// legacy form (still supported, unchanged): default ctor + SetPRAlgorithm(id)
AtPRAtask();   void SetPRAlgorithm(Int_t id = 0);
\end{lstlisting}

\begin{notebox}
The injection form is \emph{behaviour-identical} to the legacy path:
\code{AtTrackFinderTC} already carries the standard triplet-cluster defaults the
old task pushed ($s{=}0.3,\,k{=}19,\,n{=}2,\,m{=}15,\,r{=}2,\,a{=}0.03,\,t{=}4.0$,
select-and-merge on), so a migrated macro only sets the cluster radius/distance.
Validated byte-identical over 15k events by \file{testPRAinject\_a1975.C} and
\file{cmpReco\_a1975.C} (\S\ref{sec:analysis}). The $\sim$60 existing macros that
use the legacy constructor are unaffected.
\end{notebox}
...
\subsection{Fitter B --- AtGenfitter (clean genfit)}\label{sec:fitgenfitter}

\textbf{Macro:} \file{pipeline/\allowbreak fitGenfitter\_a1975.C}. Same input
(\file{<run>\_reco.root}), output \file{<run>\_genfitter<suffix>.root} (note the
\code{genfitter}, not \code{genfit}, stem). This is the \textbf{recommended genfit
path}. It uses the clean \code{EventFit::\allowbreak AtGenfitter} on the \emph{modern}
\code{AtFitterTask} --- the same wrapper as the UKF --- so the two share frame
conventions exactly. Signature:
...
The setup is deliberately small --- a constructor, two setters, and the optional
PID gate:
...
Note the \textbf{signed} field default (\code{bField=-2.85}, the a1975 experimental
handedness --- \S\ref{sec:conventions}); no separate ``simulation convention''
flag exists, the sign \emph{is} the convention. \code{elossName=""} falls back to
genfit's internal Bethe--Bloch. The vertex is taken as the highest-$z_\mathrm{digi}$
cluster end (matching PRA and the UKF), and a fitted-$\theta$ acceptance window
(default $10$--$170^\circ$) drops near-beam / backward junk.
...
root -b -q 'pipeline/fitGenfitter_a1975.C("run_0106", -1, "/mnt/f/a1975/reco/", "_pp", "", -2.85, 2, 5, "proton_H2_catima.txt", "pid/proton_band.json", 4.0, kFALSE, 2212, 1.00727646, 1)'
\end{lstlisting}

\begin{notebox}
\textbf{Why ``clean''.} \code{AtGenfitter} drops the legacy merging,
reclustering, single-vertex pooling and per-direction frame special cases that
made \code{AtGenfit} hard to reason about. Clusters are ordered by the drift
coordinate $z_\mathrm{digi}$ (monotonic along any helix, so curling tracks
sequence correctly). On a1975 \code{run\_0106} this gives $\approx 95.6\%$
convergence ($\approx 91\%$ forward) at $\sim 15\times$ the speed of the legacy
fitter; the residual non-converging fraction is dominated by PRA ``blob''
candidates, not fitter failures.
\end{notebox}

\subsubsection*{Advanced options: continuity merging and diffusion covariance}
\label{sec:advgenfit}

Two opt-in refinements extend \code{AtGenfitter} beyond the production defaults.
Both are \emph{off} by default and both leave the validated (p,p)/(p,d) results
unchanged when unused.

\textbf{Continuity merging} (\code{SetMergeContinuity}) rejoins track fragments
that PRA split across breaks but that belong to one physical trajectory, before
the fit. Two fragments merge when (i) their PRA circles are the \emph{same}
circle --- centres within \code{centerDist} (default $15$\,mm; fragments of one
helix share a centre to $\sim$10\,mm, while two distinct tracks sit $20+$\,mm
apart) and radii within \code{radiusFrac} --- and (ii) their nearest endpoints
are within \code{gapMM} in 3D and that junction is \emph{not}
vertex-end-to-vertex-end (which would be two tracks sharing the production vertex
and diverging). Chains merge transitively. It runs on a \emph{copy} of the
pattern tracks, so the shared \code{AtPatternEvent} --- and any parallel UKF task
on the same event --- is untouched.

\begin{lstlisting}
fitter->SetMergeContinuity(kTRUE);            // 15 mm / 0.3 / 30 mm defaults
\end{lstlisting}

\textbf{Diffusion covariance} (\code{SetDiffusion}, \code{SetZLongFactor},
\code{SetChargeCovRef}) replaces the flat per-cluster measurement sigma with a
drift-distance model, $\sigma^2 = \sigma_0^2 + D^2\,L_\mathrm{cm}$ (with
$\sigma_0=$\code{SetMeasSigma} and $D$ in mm$/\sqrt{\mathrm{cm}}$), so
long-drift, electron-diffused clusters are down-weighted. The defaults $(0,0)$
reduce \emph{exactly} to the flat covariance.

\begin{warnbox}
Both are physics-dependent: A/B-test before adopting (\file{featureScan\_a1975.C},
\file{testMerge\_a1975.C}, \file{mergeQuality\_a1975.C} in \S\ref{sec:analysis}).
The merge criteria were tuned by measurement on a1975; a chord-direction guard
was tried and dropped (anti-correlated on curved tracks).
\end{warnbox}

\subsection{Fitter C --- AtGenfit (legacy genfit)}

\textbf{Macro:} \file{pipeline/\allowbreak fitGenfit\_a1975.C}. Same input
(\file{<run>\_reco.root}), output \file{<run>\_genfit<suffix>.root}. The original
genfit path, kept as a validated fallback for reproducing older results. Signature:
...
Because all three read the identical \code{AtPatternEvent}, any difference in the
...
\begin{tabular}{@{}p{1.9cm}
  >{\raggedright\arraybackslash}p{3.5cm}
  >{\raggedright\arraybackslash}p{3.5cm}
  >{\raggedright\arraybackslash}p{3.5cm}@{}}
\toprule
 & \textbf{UKF} (\code{AtFitterUKF}) & \textbf{AtGenfitter} (clean) & \textbf{AtGenfit} (legacy) \\
\midrule
...
Iterations    & \code{nIter} (1--2)         & \code{minIter}--\code{maxIter} (2--5) & \code{minIter}--\code{maxIter} (5--20) \\
Field sign    & signed \code{SetBField}     & signed \code{SetBField} ($-2.85$ exp) & sim.\ convention flag \\
Re-clustering & internal / adaptive         & none; opt-in continuity merge & built-in merge / recluster \\
PID gating    & multi-hyp $\chi^2$          & \code{SetPIDGate} (Spyral PID) & --- \\
Task wrapper  & \code{AtFitterTask}         & \code{AtFitterTask}        & \code{AtFitterTaskOld} \\
...
\subsection{Overlaying both fitters (dual-fitter display)}\label{sec:dualdisp}

Because the UKF and \code{AtGenfitter} share the \code{AtPatternEvent} and the
frame conventions, their fitted trajectories can be drawn \emph{together} on the
same hits. \file{fitBoth\_a1975.C} runs both Kalman filters in one pass over a
\file{<run>\_reco.root} and writes \emph{two} tracking branches to a
\file{<run>\_both.root}:

\begin{lstlisting}
void fitBoth_a1975(TString fileName="run_0106", Long64_t nEvents=200,
                   TString ioDir="/mnt/f/a1975/reco/", TString outDir="",
                   Int_t bFieldSign=-1, Double_t bFieldMag=2.85,
                   TString pidGate="pid/proton_band.json",
                   Double_t measSigma=4.0, Double_t gasDensity=9.0e-5);
// branches: AtTrackingEventGenfit (AtGenfitter) + AtTrackingEventUKF (AtFitterUKF)
\end{lstlisting}

The display reads that file and the \file{<run>\_reco.root} hits, mapping the
lab-frame fits back to the digi frame via \code{SetZPadPlane}. The underlying
\code{AtTabFitted} tab gained the setters that make the overlay possible:
\code{SetBranchName}, \code{SetDrawSmoothed(bool)} (draw the smoothed polyline,
not the analytic helix), \code{SetTrackColor} / \code{SetLineStyle} /
\code{SetLineWidth}, and \code{SetZPadPlane}.

\begin{lstlisting}[style=bash]
# fit both, then overlay (genfit = red solid, UKF = blue dashed)
root -b -q 'pipeline/fitBoth_a1975.C("run_0106", 200, "/mnt/f/a1975/reco/", "/tmp/")'
root -l    'pipeline/run_eve_both_a1975.C("run_0106", "/mnt/f/a1975/reco/", "/tmp/")'  # interactive (needs OpenGL)
# WSL-safe static PNGs (single / multi-track / backward modes):
root -l -b -q 'pipeline/dispBoth_a1975.C("run_0106", "multi", 4, "/mnt/f/a1975/reco/", "/tmp/")'
\end{lstlisting}

\subsection{Analysis: excitation-energy spectra and the interactive explorer}
\label{sec:analysis}

Once a run is fitted, per-reaction analysis macros turn \code{AtTrackingEvent}
(plus the ion-chamber gate from the FRIB stream) into kinematics and excitation
energy. The $^{16}$C$(p,p)$ workflow lives in \file{macro/Unpack\_HDF5/a1975/UKF/pp/}
and reads the clean-genfit proton output (\file{<run>\_genfitter\_pp.root}):

\begin{longtable}{@{}p{3.2cm} p{11.4cm}@{}}
\toprule
\textbf{Macro} & \textbf{Role} \\
\midrule
\endhead
\file{cache\_pp\_run.C} & Flatten one run's fitted proton kinematics into a small
ntuple (\code{ke, theta, vz, chi2ndf, ic, run}) so the explorer never re-reads the
slow FRIB files. Drive it over all runs in parallel, then \code{hadd}. \\
\file{explore\_pp.C}    & Interactive GUI over the cached ntuple: live sliders for
beam energy, $\chi^2/\mathrm{ndf}$ and $\theta$ cuts, IC gate, and per-plot binning;
draws the Ex spectrum (Gaussian-fit elastic peak), KE-vs-$\theta_\mathrm{lab}$,
Ex-vs-$\theta_\mathrm{cm}$, and vertex-$z$-vs-Ex with kinematic-line overlays. \\
\file{prod\_ex\_pp.C}   & Batch production over many runs: IC-gated combined Ex
spectrum and elastic KE-vs-$\theta$ from the genfit protons. \\
\file{ex\_vtxcorr.C}    & Ex with a vertex energy-loss correction: self-calibrates a
linear beam-energy profile $E(z)$ (CATIMA $^{16}$C $dE/dx$) by flattening the
forward-elastic Ex tilt, then recomputes Ex per event. \\
\bottomrule
\end{longtable}
...
root -l   'pp/explore_pp.C("/tmp/pp_kin.root", 1.007825, 16.0147, "16C(p,p)")'
...
\begin{notebox}
The same explorer serves other channels by passing the ejectile/residual masses,
e.g.\ \code{explore\_pp.C("/tmp/pd\_kin.root", 2.014102, 15.010599, "16C(p,d)15C")}.
On a1975 the $(p,p)$ elastic core sits at $\sigma\!\approx\!0.18$\,MeV and is
large-angle-dominated --- i.e.\ limited by angular resolution, not beam energy
loss, which is why \file{ex\_vtxcorr.C} moves the peak only marginally.
\end{notebox}

\subsection{Regression and tuning tools}\label{sec:tools}

A set of small driver macros (in \file{pipeline/}) underpin the validation and
parameter studies above. They are not part of the production chain but are the
recommended way to A/B-test any physics-dependent knob before adopting it.
...
\midrule
\endhead
\file{testPRAinject\_a1975.C} & Prove the \code{AtPRAtask} injection ctor reproduces the legacy path (per-event track-count comparison). \\
\file{praDump\_a1975.C} / \file{cmpReco\_a1975.C} & Emit a deterministic per-event PRA signature, and diff the \code{AtPatternEvent} of two \file{\_reco.root} files --- used to show a refactor is byte-identical. \\
\file{testMerge\_a1975.C} / \file{mergeQuality\_a1975.C} / \file{mergeDiag\_a1975.C} & Validate \code{AtGenfitter} continuity merging: how often it fires, classify each merge (dedup / recover / neutral / harmful), and print the geometric discriminators per track pair. \\
\file{featureScan\_a1975.C} & Scan the genfit covariance knobs (\code{SetMeasSigma}, \code{SetDiffusion}); median $\chi^2/\mathrm{ndf}$ and good-fit rate per config. \\
\file{ukfMSscan\_a1975.C} / \file{ukfMinClus\_a1975.C} & Scan UKF multiple scattering and \code{MinClusters} versus yield / median KE. \\
\bottomrule
\end{longtable}
...
\code{AtFitterTask} owns any \code{EventFit::\allowbreak AtFitter} and drives it per event.
...
\code{SetEnableMultipleScattering(b)}  & Highland multiple-scattering angular process noise (false). Opt-in; negligible in H$_2$ at 1\,bar, scales for denser gas / lower momentum. \\
\code{SetRadiationLength(mm)}          & Radiation length $X_0$ of the medium used by the Highland model ($7.6\times10^{6}$\,mm, $\approx$H$_2$ at 1\,bar). \\
\bottomrule
\end{longtable}
...
\subsection{\texttt{EventFit::AtGenfitter} (clean genfit)}

\textbf{Constructor} (all arguments defaulted):
\begin{lstlisting}
...
\textbf{Setters} (defaults in parentheses):

\begin{longtable}{@{}p{6.6cm} p{7.8cm}@{}}
\toprule
\textbf{Method} & \textbf{Meaning} \\
\midrule
\endhead
\code{SetParticle(pdg, massAmu, Z)}     & Override the particle hypothesis. \\
\code{SetBField(tesla)}                 & Signed field, Tesla ($-2.85$). Sign $=$ handedness, see \S\ref{sec:conventions}. \\
\code{SetZPadPlane(z)}                  & Pad-plane $z$ (mm); $z_\mathrm{lab}=z-z_\mathrm{digi}$ (1000). \\
\code{SetMinClusters(n)}                & Skip candidates with fewer clusters (4). \\
\code{SetIterations(mn, mx)}            & Min / max Kalman iterations (2, 5). \\
\code{SetMeasSigma(mm)}                 & Per-cluster measurement resolution (4.0); absorbs scattering when material effects are off. \\
\code{SetVertexAxisMaxDist(mm)}         & Keep only tracks whose closest cluster is within this of the beam axis (150). \\
\code{SetThetaWindow(min, max)}         & Fitted-$\theta$ acceptance window in deg ($10$--$170$); outside $\Rightarrow$ track dropped. \\
\code{SetQualityCut(chi2NdfMax, keMin, keMax)} & Post-fit flag only ($5.0$, $0.5$, $60$); failing tracks are kept but \code{GetGoodFit()==false}. \\
\code{SetPIDGate(jsonFile)}             & Fit only tracks inside a Spyral-PID gate, see \S\ref{sec:pidgate}. \\
\code{SetMergeContinuity(on, centerDist, radiusFrac, gapMM)} & \emph{Opt-in} (default off). Merge PRA-split fragments of one track before fitting --- see \S\ref{sec:advgenfit}. ($15$\,mm, $0.3$, $30$\,mm). \\
\code{SetDiffusion(transv, long)}       & Drift-distance diffusion model for the per-cluster covariance, mm$/\sqrt{\mathrm{cm}}$ ($0,0$ $\Rightarrow$ flat \code{SetMeasSigma}). \\
\code{SetZLongFactor(f)}                & Drift-$z$ variance looseness at zero drift (2.0). \\
\code{SetChargeCovRef(qRef)}            & If $>0$, weight clusters by charge (clamp $q_\mathrm{ref}/Q$); off (0). \\
\bottomrule
\end{longtable}

It uses \code{genfit::KalmanFitterRefTrack} with \code{RKTrackRep} and
\code{AtSpacepointMeasurement}, and (like the legacy fitter) requires a loaded
\code{TGeoManager}. Clusters are ordered by drift coordinate $z_\mathrm{digi}$;
the vertex is the highest-$z_\mathrm{digi}$ end (lowest $z_\mathrm{lab}$), matching
PRA and the UKF.

\subsection{\texttt{AtFITTER::AtGenfit} (legacy)}
...
% =====================================================================
\section{Particle identification and the PID gate}\label{sec:pidgate}
% =====================================================================

Particle identification uses a C++ port of Spyral's algorithm. \code{AtPIDTask}
(class \code{AtTools::AtSpyralPID} / \code{AtPIDEstimator}) reads
\code{AtPatternEvent} and writes \code{AtPIDEvent}, a per-track set of observables
--- magnetic rigidity $B\rho$, a truncated-mean $\sqrt{dE/dx}$ on the denoised PRA
clusters, integrated charge, and cluster count. Plotting $\sqrt{dE/dx}$ versus
$B\rho$ separates species into the familiar PID bands.
...
A 2D acceptance region is stored as an \code{AtParticleID} JSON gate (a polygon in
the $(\sqrt{dE/dx}, B\rho)$ plane), e.g.\ \file{pid/deuteron\_band.json} or
\file{pid/proton\_band.json}.

\subsection{In-fitter gating with \texttt{SetPIDGate}}

\code{EventFit::AtGenfitter::SetPIDGate(jsonFile)} makes the fitter compute the
\emph{same} Spyral PID internally (it owns its own \code{AtSpyralPID}, so it needs
no \code{AtPIDEvent} branch) and skip any track whose PID falls outside the gate
\emph{before} the expensive genfit fit. This both speeds up production --- only
the species of interest is fitted --- and removes most mis-identified candidates
from the output. The gate is purely an input filter; tracks it rejects never
appear in \code{AtTrackingEvent}.
...
\begin{notebox}
Match the gate to the hypothesis: a proton fit (\code{pdg=2212}) should use
\code{pid/proton\_band.json}, a deuteron fit \code{pid/deuteron\_band.json}. If the
JSON path does not exist the macro warns and disables gating (every candidate is
fitted), so a silently mistyped path simply removes the speed-up rather than
crashing.
\end{notebox}
...
For the clean \code{AtGenfitter} the control is identical to the UKF: it takes a
\textbf{signed} field, so pass \code{bField=-2.85} for experiment (the default) and
\code{+2.85} for simulation --- there is no separate convention flag. The legacy
\code{AtGenfit} instead uses \code{SetSimulationConvention\allowbreak (false)} for experimental
data.
...
\subsection*{Experiment (a1975), genfit (clean, recommended)}
\begin{lstlisting}[style=bash]
cd macro/Unpack_HDF5/a1975/UKF/
...
\subsection*{Experiment (a1975), genfit (legacy fallback)}
\begin{lstlisting}[style=bash]
cd macro/Unpack_HDF5/a1975/UKF/
root -b -q 'pipeline/fitGenfit_a1975.C("run_0106", -1, "deuteron")'   # -> run_0106_genfit.root
\end{lstlisting}

\subsection*{Experiment (a1975), overlay both fitters}
\begin{lstlisting}[style=bash]
cd macro/Unpack_HDF5/a1975/UKF/
root -b -q 'pipeline/fitBoth_a1975.C("run_0106", 200, "/mnt/f/a1975/reco/", "/tmp/")'  # -> _both.root
root -l -b -q 'pipeline/dispBoth_a1975.C("run_0106", "multi", 4, "/mnt/f/a1975/reco/", "/tmp/")'
\end{lstlisting}

\subsection*{One-shot experiment (no iteration)}
\begin{lstlisting}[style=bash]
# UKF one-shot (in macro/Unpack_HDF5/a1975/UKF/)
root -b -q 'pipeline/unpackNFit_a1975_UKF.C("run_0116", 1000, "proton", -1, 2.85, 9.0e-5)'
# genfit one-shot (in macro/Unpack_HDF5/a1975/): (p,p) proton / (p,d) deuteron
root -l -b -q 'unpackNFit_a1975.C("run_0106", 5000)'
root -l -b -q 'unpackNFit_a1975_deuterium.C("run_0106", 5000)'
\end{lstlisting}
...
\begin{description}[leftmargin=0.5cm,style=nextline]
  ...
  \code{SetSimulationConvention\allowbreak (false)} (genfit). The elastic recoil locus should
  ...
  (\S\ref{sec:env}); without it neither \code{AtGenfitter} nor \code{AtGenfit} is
  compiled and only the UKF is available.
  \item[Which genfit fitter to run]
  Use the clean \code{AtGenfitter} (\file{fitGenfitter\_a1975.C}) for all new work
  --- it is faster, shares the UKF conventions, and supports PID gating. Fall back
  to the legacy \code{AtGenfit} (\file{fitGenfit\_a1975.C}) only to reproduce older
  results. Mind the output stems: \file{\_genfitter} vs \file{\_genfit}.
  \item[PID gate ``not found''; everything still fitted]
  The \code{pidGate} path passed to \file{fitGenfitter\_a1975.C} does not resolve,
  so gating is disabled (a warning is printed) and every candidate is fitted. Pass
  a gate that matches the hypothesis (\file{pid/proton\_band.json} for a proton
  fit, \file{pid/deuteron\_band.json} for a deuteron fit); see \S\ref{sec:pidgate}.
