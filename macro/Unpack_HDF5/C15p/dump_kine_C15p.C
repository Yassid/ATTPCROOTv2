   ...
   for (Long64_t i = 0; i < t->GetEntries(); ++i) {
      ...
      for (auto &ft : ev->GetFittedTracks()) {
         if (!ft)
            continue;
         ++nFit;
         // Prefer the event number carried on the event itself. For a fit run on GATED input the
         // tree index is a gated-file index, not the raw event, so every downstream join on
         // (run, event, trackID) would mismatch. pid/gate_events_C15p.C stamps the original index
         ...
         // ★ THE VERTEX COMES FROM GetVertex(), NOT TrackProperties.initialPositionXtr.
         // initialPositionXtr is filled by the UKF fitter; AtGenfitter never assigns it, so
         // reading it here gave EXACTLY (0,0,0) for every one of 1,018,172 GENFIT tracks and the
         // viewer hid its Ex-vs-z panel as "no usable column". The GENFIT path does compute the
         // vertex -- AtGenfitter.cxx:668 extrapolates the fitted state back to the beam axis and
         // stores it with SetVertex(), in mm. It was there all along, in the other field.
         // fVertex can legitimately be empty if the fit never produced one, and GetVertex() is
         // .at(0), so guard rather than let it throw mid-run.
