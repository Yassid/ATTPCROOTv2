/// @file ebeam_pp_C15p.C
/// @brief Measure the 15C beam energy from the 15C(p,p)15C elastic recoil ridge.
///
///   root -b -q 'pp/ebeam_pp_C15p.C()'
///
...
const double kMd = 1.00782503 * kU; // PROTON target and ejectile (15C(p,p'))
...
void ebeam_pp_C15p(TString kinFile = "pp/plots/pp_kin_C15p.root", TString outDir = "pp/plots/",
                   Double_t thLo = 42, Double_t thHi = 78, Double_t thStep = 2.0,
                   Double_t seedTheta = 50.0,
                   /// ★ KE FLOOR. It exists to keep the per-slice ridge finder off a low-energy
                   /// bulk, and its right value is CHANNEL-SPECIFIC. C15d needs 24 MeV because its
                   /// bulk peaks near 4 MeV with a tail to ~20. Here the (p,p') elastic ridge is
                   /// itself at ~18.7 MeV at 51 deg, so a 24 MeV floor cuts away the very ridge it
                   /// is meant to find -- ported unchanged from C15d it produced "no ridge in the
                   /// seed slice". This sample has no comparable bulk, so the answer is flat in the
                   /// floor: 3 -> 196, 5 -> 196, 8 -> 201, 12 -> 200, 16 -> 201 MeV, against the
                   /// locus-count estimator's 202. DO NOT copy this number between channels.
                   Double_t keMinUse = 8.0)
{
   gSystem->mkdir(outDir, kTRUE);
   if (gSystem->AccessPathName(kinFile)) {
      std::cout << "\033[1;31mERROR: " << kinFile << " not found (run pp/kin_pp_C15p.C).\033[0m\n";
      return;
   }
   TFile f(kinFile);
   ...
   std::cout << "\033[1;33m=== 15C(p,p) elastic ridge ===\033[0m\n  walked " << g->GetN() << " slices\n";
   ...
   g->SetTitle("15C(p,p) elastic recoil ridge;#theta_{lab} [deg];KE_{d} [MeV]");
   g->SetMarkerStyle(20);
   g->Draw("AP");
   if (!ebs.empty()) {
   ...
   c->SaveAs(outDir + "ebeam_pp_C15p.png");
   std::cout << "  wrote " << outDir << "ebeam_pp_C15p.png\n";
}
