...
///
/// Chains every <run>_kin_p.root, keeps the tracks in the proton selection (joined on
...
void kin_pp_C15p(TString fitDir = "/home/yassid/C15p_fit/", TString selFile = "pid/sel_proton_C15p.root",
                 TString outDir = "pp/plots/", Double_t chi2Cut = 5.0, Double_t keMax = 60.0,
                 ...
                 TString pointsFile = "pid/points_C15p.root")
{
   ...
   if (selFile.Length()) {
      if (gSystem->AccessPathName(selFile)) {
         std::cout << "\033[1;31mERROR: " << selFile << " not found (run apply_gate_C15p.C).\033[0m\n";
         return;
      }
