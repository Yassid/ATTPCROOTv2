                      ...
                      Double_t bField = 2.85, Double_t thMinDeg = -1, Bool_t requireSinglePulse = kTRUE)
{
   ...
   auto gain = LoadGainTable_C15p(gt, false);
   bool missingGain = false;
   const double gf = GainFactor_C15p(gain, runNo, missingGain);
   if (missingGain) {
   ...
   // ---- IC keep-flags, with the same length check make_points_C15p.C uses --------------------
