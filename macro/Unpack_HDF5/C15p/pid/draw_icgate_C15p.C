...
///   root -l 'pid/draw_icgate_C15p.C("ic_C15p")'                // pick with the mouse
///   root -l 'pid/draw_icgate_C15p.C("ic_C15p",931,1413)'      // or set the window directly
///   root -l 'pid/draw_icgate_C15p.C("ic_peak2",1792,2400)'    // the second beam component
///
...
void draw_icgate_C15p(TString name = "ic_C15p", double presetLo = -1, double presetHi = -1,
                     TString icDir = "/home/yassid/a2091_C15_ic/", bool singlePulse = true, double lo = 600,
                      double hi = 2600)
{
   gStyle->SetOptStat(0);
   ...
   if (!(gLo > 0 && gHi > gLo)) {
      ...
      printf("  You can always set the window directly instead:\n"
             "    root -l 'pid/draw_icgate_C15p.C(\"%s\",931,1413)'\n",
             name.Data());
      return;
   }
