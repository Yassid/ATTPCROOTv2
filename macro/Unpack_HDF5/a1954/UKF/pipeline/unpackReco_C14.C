/// @file unpackReco_C14.C
/// @brief a1954 14C(p,p') reco stage: unpack -> AtPSAMultiFit -> (clean) -> PRA.
///
/// Ported from a1975 D2_UKF/unpackReco_multifit.C for the 14C proton-target runs.
/// Produces <run>_reco.root (AtEventH point cloud + AtPatternEvent track candidates),
/// which the fast fitUKF_C14.C / GENFIT stages then fit.
///
/// Differences vs the a1975 macro (flagged):
///   * parFile     = ATTPC.a1954_C14.par  (B=2.85T, H2 600 torr, drift 1.00 cm/us)
...
///
///   root -l 'unpackReco_C14.C("run_0055")'        // whole run
///   root -l 'unpackReco_C14.C("run_0055", 500)'   // first 500 events
void unpackReco_C14(TString fileName = "run_0055", Long64_t nEvents = 0, Bool_t persistRaw = true,
                     TString outDir = "/home/yassid/a1954_C14_reco/",
                     TString filepath = "/mnt/f/a1954_remerged/", Bool_t doSC = false,
                     ...
                     Double_t relErr = 0.1, TString parFile = "ATTPC.a1954_C14.par", Bool_t doClean = kTRUE)
{
   ...
   if (applyTimeCorr)
      psa->LoadPadTimeOffsets((dir + "/macro/Unpack_HDF5/a1954/UKF/pipeline/pad_time_correction.csv").Data());
