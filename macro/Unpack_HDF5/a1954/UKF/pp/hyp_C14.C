/// @file hyp_C14.C
/// @brief Decide WHAT causes the Ex-vs-angle correlation of the a1954 14C(p,p') elastic peak.
///
/// The elastic Ex drifts by ~0.4 MeV across the angular range. Three hypotheses flatten it,
/// and they are NOT equivalent — they put the known 14C excited states in different places,
...
///   (C) empirical THETA_CM correction (excorr_C14.C): flattens by construction, says nothing
///                                about the cause
///
/// For each, the macro solves for the parameter that flattens the elastic ridge and then
/// reports where the excited group lands. Known 14C levels: 6.09(1-), 6.59(0+), 6.73(3-),
/// 6.90(0-), 7.01(2+), 7.34(2-), 8.32.
///
///   root -b -q 'pp/hyp_C14.C("plots/proton_kin_5run_ukf.root",161,"ukf")'
...
void hyp_C14(TString cache = "plots/proton_kin_5run_ukf.root", double Ebeam0 = 161.0, TString tag = "ukf",
             double chi2Cut = 5.0)
{
   gStyle->SetOptStat(0);
   ...
   const double U = 931.49401, m1 = 14.003242 * U, m2 = 1.007825 * U, m3 = 1.007825 * U, m4 = 14.003242 * U;
   Float_t ke, th, c2;
   ...
   printf("    known 14C levels near the group: 6.09(1-) 6.59(0+) 6.73(3-) 6.90(0-) 7.01(2+) 7.34(2-)\n");
}
