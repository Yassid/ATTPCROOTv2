...
/// @brief Bake kinematics caches (proton_kin*.root from pp/ex_C14.C) into a single
/// self-contained HTML file: the same explorer as pp/explore_C14.C, but running in a
...
///
///   root -b -q 'pp/make_explorer_html.C()'      // clean161 UKF cache, 161 MeV
///   root -b -q 'pp/make_explorer_html.C("plots/proton_kin_clean161.root","/home/yassid/pp.html",\
///                "14C(p,p'\'')",161,14.003242,1.007825,1.007825,14.003242,12,"",\
///                "plots/proton_kin_clean161_genfit.root")'
///
...
static Long64_t dump_pk(TTree *t, FILE *o)
{
   ...
   // vertexz is absent from very old caches, and was hardcoded to 0 in ex_C14.C before
...
void make_explorer_html(TString cache = "", TString outHtml = "", TString tag = "14C(p,p')",
                        double ebeam0 = 161.0, double mBeamAmu = 14.003242, double mTargAmu = 1.007825,
                        double mEjectAmu = 1.007825, double mResidAmu = 14.003242, int beamA = 14,
                        TString refExCSV = "", TString cacheGenfit = "")
{
   TString here = gSystem->DirName(gInterpreter->GetCurrentMacroName());
   if (cache.IsNull())
      cache = here + "/plots/proton_kin_clean161.root";
   if (outHtml.IsNull())
      outHtml = TString(gSystem->Getenv("HOME")) + "/a1954_C14_explorer.html";
   ...
   if (refExCSV.IsNull())
      refExCSV = (mResidAmu > 13.5) ? "0:g.s.,6.09:1-,6.59:0+_2,7.34:2+"   // 14C
