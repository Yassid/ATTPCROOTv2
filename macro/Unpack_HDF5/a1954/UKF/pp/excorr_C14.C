/// @file excorr_C14.C
/// @brief Flatten the Ex-vs-theta_cm dependence of the a1954 14C(p,p') elastic peak.
///
...
///
///   root -b -q 'pp/excorr_C14.C("plots/proton_kin_5run_ukf.root",161,"ukf")'
static double omegaE(double x, double y, double z)
{
   return std::sqrt(x * x + y * y + z * z - 2 * x * y - 2 * y * z - 2 * x * z);
}

void excorr_C14(TString cache = "plots/proton_kin_5run_ukf.root", double Ebeam = 161.0, TString tag = "ukf",
                double chi2Cut = 5.0, int nPol = 2, double tcLo = 15, double tcHi = 75, double gsWin = 1.2)
{
   ...
   h1b->DrawCopy();
   TString png = here + "/plots/excorr_C14_" + tag + ".png";
   c->SaveAs(png);
   printf("saved %s\n", png.Data());
}
