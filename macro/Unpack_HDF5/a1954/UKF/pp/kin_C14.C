/// @file kin_C14.C
/// @brief Kinematics check for a1954 14C(p,p'): the measured recoil-proton KE-vs-theta_lab
///        locus with the two-body loci of the known 14C levels drawn on top, the Ex spectrum,
///        and a Newton solve for the beam energy that puts the elastic peak at Ex = 0.
///
///        Runs on the ntuple cache written by pp/ex_C14.C (ke:theta:vertexz:thcm:ex:chi2ndf),
///        so it costs nothing to re-run while more runs finish.
///
///   root -b -q 'pp/kin_C14.C("plots/proton_kin_first_ukf.root",161,"ukf")'
...
// Ex from (theta_lab, KE) for m1(m2,m3)m4 -- same expression as ex_C14.C
...
void kin_C14(TString cache = "plots/proton_kin_first_ukf.root", double Ebeam = 161.0, TString tag = "ukf",
             double chi2Cut = 5.0, double mEjectAmu = 1.007825, double mResidAmu = 14.003242)
{
   gStyle->SetOptStat(0);
   ...
   TH2F *hkt = new TH2F("hkt", TString::Format("a1954 14C(p,p') %s: recoil-proton KE vs #theta_{lab} (E_{beam}=%.1f "
   ...
   TString png = here + "/plots/kin_C14_" + tag + ".png";
   c->SaveAs(png);
   printf("saved %s\n", png.Data());
}
