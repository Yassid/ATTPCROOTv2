#!/bin/bash
# 14C(p,d)13C pass: per run  gate (IC 500-900 + DEUTERON-gated tracks) -> UKF(deuteron).
...
#   ./fitpipe_pd_C14.sh "run_0143 run_0147" 4 [thMin]
...
FITDIR="/home/yassid/a1954_C14_fit_pd/"; IN="${FITDIR}in/"; LOG="${FITDIR}logs"; mkdir -p "$IN" "$LOG"
DGATE="$HERE/pid/deuteron_14C.json"
...
one(){ local r="$1"; local L="$LOG/${r}.log"
  ...
  root -b -q -l "$HERE/pipeline/gate_events_C14.C(\"$r\",\"$SLIM\",\"$IN\",\"$FREF${r}_reco.root\",500,900,1050,1250,2.85,\"$DGATE\",$THMIN)" > "$L" 2>&1
  [ -f "$IN/${r}_reco.root" ] || { echo "gate failed $r"; return; }
  # 2) UKF with the DEUTERON mass hypothesis
  root -b -q -l "$HERE/pipeline/fitUKF_C14.C(\"$r\",-1,\"deuteron\",-1,2.85,6.5e-5,\"\",\"$IN\",0.5,0.1,1,10,\"$FITDIR\")" >> "$L" 2>&1
