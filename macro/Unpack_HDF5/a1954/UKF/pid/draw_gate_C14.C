/// @file draw_gate_C14.C
/// @brief INTERACTIVE PID gate drawing for a1954 14C. Shows the (IC-gated) PID plane and
///        lets you draw a polygon with the mouse; saves it as an AtParticleID JSON gate that
///        the analysis macros read (dump_pid_C14.C, ic_gated_pid_C14.C, etc).
///
/// USAGE (run interactively, NOT with -b):
///   root -l 'draw_gate_C14.C("proton_14C")'                 // proton gate (Brho axis to 1.2)
///   root -l 'draw_gate_C14.C("deuteron_14C",950,1350,0,1,2)' // deuteron gate (Z=1,A=2)
///
...
///       icLo,icHi = IC beam gate (14C = 950-1350; set icLo<0 to use ALL beam)
...
void draw_gate_C14(TString name = "proton_14C", double icLo = 950, double icHi = 1350, double arclenMin = 0,
                    int Z = 1, int A = 1, double sdMax = 55, double brMax = 1.2,
                    TString dataFile = "pid_C14_data.csv")
{
   gStyle->SetOptStat(0);
   ...
   TH2F *h = new TH2F("h", TString::Format("a1954 14C PID  -  draw the %s gate;#sqrt{dEdx};B#rho [T m]", name.Data()),
