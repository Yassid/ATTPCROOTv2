/// @file brho_theta_C14.C
/// @brief Bρ vs θ_lab for a1954 14C from PRA track candidates (AtSpyralPID, no fit).
///        Quick-look kinematic locus straight off <run>_reco.root.
///
///   root -b -q 'pid/brho_theta_C14.C("run_0055,run_0058", "/mnt/f/a1954_C14_reco_hdb/")'
void brho_theta_C14(TString runsCSV = "run_0055", TString inDir = "/mnt/f/a1954_C14_reco_hdb/", Long64_t nEvents = -1,
                     double bField = 2.85, double arclenMin = 0, TString outTag = "", double brhoMax = 2.5)
{
   ...
   TH2F *hBt = new TH2F("hBt", "14C  B#rho vs #theta_{lab}  (PRA tracks, Spyral);#theta_{lab} [deg];B#rho [T m]", 180,
                        0, 180, 400, 0, brhoMax);
   ...
   printf("\n== 14C Brho-vs-theta ==  runs=%ld  tracks=%ld  valid=%ld (%.1f%%)  arclenMin=%.0f\n", nRuns, nTracks,
          nValid, nTracks ? 100.0 * nValid / nTracks : 0, arclenMin);

   TCanvas *c = new TCanvas("c", "brho_theta", 900, 700);
   c->SetRightMargin(0.13);
   hBt->Draw("colz");
   TString png = plotDir + "brho_theta_C14" + outTag + ".png";
   c->SaveAs(png);
   printf("saved %s\n", png.Data());
}
