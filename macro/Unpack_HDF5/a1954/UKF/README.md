# a1954 14C(p,p') — UKF / GENFIT workspace

Modern staged reconstruction of the **a1954 14C proton-target runs**, ported from the
a1975 16C `UKF/` pipeline. Reaction: **14C(p,p')14C*** (inverse kinematics, 14C beam
...
- Raw HDF5: `/mnt/f/a1954_remerged/run_XXXX.h5`
  (0142 is 0.6 GB; 0143–0215 are ~5–9 GB each).
- Parameters: `parameters/ATTPC.a1954_C14.par` (B=2.85 T, H2 600 torr, drift 1.00 cm/µs).
- Reco/fit outputs: `/home/yassid/a1954_C14_reco/` (+ `logs/`).

## Pipeline

| Macro | Does | Output |
|-------|------|--------|
| `pipeline/unpackReco_C14.C(run, nEv, ...)` | unpack + **AtPSAMultiFit** + **AtDirDeDxCleaner** + PRA(triplclust) | `<run>_reco.root` (AtEventH + AtPatternEvent) |
| `pipeline/fitUKF_C14.C(run, nEv, particles, bFieldSign)` | **new UKF** (AtFitterUKFMulti) — fast, reads `_reco.root` | `<run>_ukf.root` (AtTrackingEvent) |
| `pipeline/fitGenfit_C14.C(run, nEv, ioDir, ...)` | **updated GENFIT** (AtGenfitter) — needs the genfit build | `<run>_genfit.root` (AtTrackingEvent + AtPIDEvent) |
| `pp/ex_C14.C(runsCSV, inDir, Ebeam)` | two-body kinematics → 14C excitation spectrum + KE-vs-θ | `pp/plots/ex_C14.png` + `pp/plots/proton_kin<tag>.root` (cache) |
| `pp/explore_C14.C(cache, ...)` | **interactive GUI** on that cache: Ebeam / cuts / all binnings, Ex recomputed live, `Zero g.s.` solves Ebeam | `pp/plots/explore_C14.png` |

| `pp/make_explorer_html.C(cache, out, ...)` | bakes a cache into a standalone **browser** explorer (same controls, no X11) | `~/a1954_C14_explorer.html` |
...
```bash
root -b -q '<...>/pp/make_explorer_html.C()'      # bake cache -> ~/a1954_C14_explorer.html
explorer.exe ~/a1954_C14_explorer.html           # opens in the Windows browser
```
...
that `ex_C14.C` kept (26,597 vs 26,604).

Interactive explorer — plain ROOT is enough (no `config.sh`, no `VMCWORKDIR`; it finds
`plots/proton_kin_clean161.root` next to itself). Must run **without** `-b`:
```bash
root -l ~/fair_install/ATTPCROOTv2-OpenKF/macro/Unpack_HDF5/a1954/UKF/pp/explore_C14.C
```
...
(`bFieldSign = -1`, the default). Confirmed on run_0055 (proton hypothesis):
...
1. **Beam energy** — `ex_C14.C` defaults `Ebeam = 100 MeV` as a PLACEHOLDER. The
   absolute Ex scale depends on it. Set the real a1954 14C beam energy.
2. **Gas density for CATIMA** — `fitUKF_C14.C` defaults 6.5e-5 g/cm³ (H2 @ 600 torr,
...
4. **No IC beam gate / no proton PID gate yet** — `ex_C14.C` uses a fit-quality cut
   only. Add: (a) FRIB IC gate via the `_FRIBDAQ` unpacker → `<run>_FRIB.root`;
   (b) a 14C proton gate from the sqrt(dEdx)-vs-Brho plane (AtSpyralPID).
...
root -b -q 'pipeline/unpackReco_C14.C("run_0055")'
# UKF fit (fast, iterate)
root -b -q 'pipeline/fitUKF_C14.C("run_0055", -1, "proton", -1)'
