/// @file kin_dp_C15d.C
/// @brief (d,p) kinematics: map KE vs theta for the gated protons, and cache them for the explorer.
///
///   root -b -q 'dp/kin_dp_C15d.C()'
///
/// Chains every <run>_kin_p.root, keeps the tracks in the deuteron selection (joined on
/// run/event/trackID), and writes the KE-vs-theta_lab map plus a compact `dd` ntuple.
///
/// ★ WHAT THIS CHANNEL IS FOR, and it is NOT the beam energy. 15C(d,p)16C leaves a 16C residual,
/// whose level scheme is nothing like 15C's -- 16C's first excited state is the 2+ at 1.766, where
/// 15C has 0.740, 3.103, 4.220, 4.657. THAT is what makes (d,p) the test which actually
/// discriminates the beam: the elastic ridge and the (d,d') Ex spectrum are both nearly blind to
/// beam mass, because there the beam and the residual are the same nucleus. See
/// [[attpcroot-c15d-ex-puzzle]] for how that degeneracy wasted a week on the wrong dataset.
///
/// The two-body expression below is kept for reference; for the ELASTIC channel it reads:
///
...
void kin_dp_C15d(TString fitDir = "/home/yassid/C15d_fit/", TString selFile = "",
                 TString outDir = "dp/plots/", Double_t chi2Cut = 5.0, Double_t keMax = 150.0,
                 ...
                 TString pointsFile = "pid/points_C15d.root")
{
   ...
   auto *h = new TH2D("hdd", "15C(d,p) gated protons;#theta_{lab} [deg];KE [MeV]", 180, 0, 180, 240, 0, keMax);
   auto *hAll = new TH2D("hddAll", "all fitted deuterons;#theta_{lab} [deg];KE [MeV]", 180, 0, 180, 240, 0, keMax);

   TFile fo(outDir + "dp_kin_C15d.root", "RECREATE");
   TTree dd("dd", "gated (d,p) kinematics");
   ...
   std::cout << "\033[1;33m=== (d,p) kinematics ===\033[0m\n"
             << "  kin files  : " << nf << "\n"
             ...
             << "  \033[1;32mwrote\033[0m " << outDir << "dp_kin_C15d.{root,png}\n";
}
