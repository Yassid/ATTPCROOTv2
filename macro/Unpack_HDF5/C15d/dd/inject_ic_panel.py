#!/usr/bin/env python3
"""Add the ion-chamber panel to a generated C15d explorer page.

    python3 dd/inject_ic_panel.py <explorer.html> <ic.json> [out.html]

The explorer template is shared with the other channels, so the IC panel is INJECTED into the
generated page rather than added to the template -- that keeps a C15d-only panel out of every
...
is the one the gate is defined on (pid/ic_C15d.json records singlePulse: true).
"""
import json
import re
import sys
...
SCRIPT = '''
<script>
/* ---- ion-chamber panel (injected by dd/inject_ic_panel.py) -------------------------------
...
(function(){
  ...
  /* The panel does not own the selection -- the page's icLo/icHi inputs do, so the window the
     ...
     for a re-render. icLo < 0 means no window, matching apply_gate_C15d.C's convention. */
  ...
  function draw(){
    ...
    // the window recorded in pid/ic_C15d.json, marked but never enforced, so a hand-dragged
