...
///
///   root -b -q 'pp/make_explorer_dd_C15d.C()'      // clean195 UKF cache, 161 MeV
///   root -b -q 'pp/make_explorer_dd_C15d.C("plots/proton_kin_clean195.root","/home/yassid/pp.html",\
...
void make_explorer_dd_C15d(TString cache = "", TString outHtml = "", TString tag = "15C(d,d')",
                        // Ebeam from THIS analysis's own 15C(d,d) elastic ridge (dd/ebeam_dd_C15d.C),
                        // not an external number -- so the g.s. landing at Ex = 0 here is a
                        // self-consistency check, not a calibration.
                        double ebeam0 = 90.0, double mBeamAmu = 15.0105993, double mTargAmu = 2.0141018,
                        // (d,d'): the ejectile is the DEUTERON and the residual is 15C itself.
                        double mEjectAmu = 2.0141018, double mResidAmu = 15.0105993, int beamA = 15,
                        ...
                        TString setNamesCSV = "")
{
   ...
   if (cache.IsNull())
      cache = here + "/plots/dd_kin_C15d.root";
   ...
   if (outHtml.IsNull())
      outHtml = TString(gSystem->Getenv("HOME")) + "/C15d_dd_explorer.html";
   ...
   if (tT)
      printf("  %-10s : %lld tracks\n", sn[2].Data(), nT);
   printf("open it with:  dd/open_explorer_dd.sh   (or any browser on %s)\n\n", outHtml.Data());
}
