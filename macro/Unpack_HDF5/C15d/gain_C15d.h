/// @file gain_C15d.h
/// @brief Load a C15d gain-match table and apply it at read time.
///
...
inline std::map<int, double> LoadGainTable_C15d(const TString &path, bool verbose = true)
{
...
inline double GainFactor_C15d(const std::map<int, double> &table, int run, bool &missing)
{
   if (table.empty()) {
