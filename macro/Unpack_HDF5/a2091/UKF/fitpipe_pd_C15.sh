#!/bin/bash
# 15C(p,d)14C pass: per run  gate (IC 500-900 + DEUTERON-gated tracks) -> UKF(deuteron).
...
FITDIR="/home/yassid/a2091_C15_fit_pd/"; IN="${FITDIR}in/"; LOG="${FITDIR}logs"; mkdir -p "$IN" "$LOG"
...
one(){ local r="$1"; local L="$LOG/${r}.log"
  ...
  root -b -q -l "$HERE/pipeline/gate_events_C15.C(\"$r\",\"$SLIM\",\"$IN\",\"$FREF${r}_reco.root\",500,900,1050,1250,2.85,\"$DGATE\",$THMIN)" > "$L" 2>&1
  [ -f "$IN/${r}_reco.root" ] || { echo "gate failed $r"; return; }
  # 2) UKF with the DEUTERON mass hypothesis
  root -b -q -l "$HERE/pipeline/fitUKF_C15.C(\"$r\",-1,\"deuteron\",-1,2.85,3.553e-5,\"\",\"$IN\",0.5,0.1,1,10,\"$FITDIR\")" >> "$L" 2>&1
