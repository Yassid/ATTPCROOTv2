/// @file explore_C15.C
/// @brief Interactive explorer for the a2091 15C(p,p') UKF protons. Loads the cached
/// kinematics ntuple written by pp/ex_C15.C (e.g. plots/proton_kin_clean195.root) into
...
///   root -l /home/yassid/fair_install/ATTPCROOTv2/macro/Unpack_HDF5/a2091/UKF/pp/explore_C15.C
///
/// Another cache / another channel, e.g. 15C(p,d)14C:
///   root -l 'pp/explore_C15.C("plots/proton_kin_pd.root",2.014102,13.003355,"15C(p,d)14C")'
///
/// Controls
...
/// two-body kinematics (same expression as ex_C15.C): returns {Ex, theta_cm[deg]}
...
/// ejectile KE vs lab angle locus for p(15C,15C*)p at excitation Ex (CM-angle scan)
...
class C15Explorer : public TObject {
public:
   C15Explorer(TString cache, double mEjectAmu, double mResidAmu, TString tag, double Ebeam0)
   {
      ...
      if (!f || f->IsZombie()) {
         std::cerr << "\n\033[1;31m[explore_C15] cannot open cache file:\033[0m " << cache << "\n"
                   << "  Build it first with pp/ex_C15.C, or pass the path explicitly:\n"
                   << "    root -l 'explore_C15.C(\"/path/to/proton_kin_clean195.root\")'\n\n";
         return;
      }
      TTree *t = (TTree *)f->Get("pk");
      if (!t) {
         std::cerr << "\n\033[1;31m[explore_C15] no ntuple 'pk' in\033[0m " << cache << "\n\n";
         return;
      }
      ...
      f->Close();
      std::cout << "C15Explorer: loaded " << fKe.size() << " ejectiles from " << cache << "\n";
      MakeGui(Ebeam0);
      Redraw();
   }
...
private:
   ...
   TGNumberEntry *mkNum(TGCompositeFrame *bar, const char *lab, double val, double lo, double hi, int dig = 0)
   {
      ...
      ne->Connect("ValueSet(Long_t)", "C15Explorer", this, "Redraw()"); // arrows redraw immediately
   ...
   ClassDef(C15Explorer, 0);
};

/// defaults = 15C(p,p') on the a2091 15C+p cache at 161 MeV (11.5 MeV/u)
void explore_C15(TString cache = "", double mEjectAmu = 1.007825, double mResidAmu = 15.0105993,
                  TString tag = "15C(p,p')", double Ebeam0 = 195.0)
{
   gStyle->SetOptStat(0);
   ...
   if (gROOT->IsBatch()) {
      std::cerr << "\n\033[1;31m[explore_C15] ROOT is in BATCH mode -- no window can open.\033[0m\n"
                << "  Start it WITHOUT -b:   root -l " << macroDir() << "/explore_C15.C\n\n";
      return;
   }
   if (!gSystem->Getenv("DISPLAY")) {
      std::cerr << "\n\033[1;31m[explore_C15] DISPLAY is not set -- no X server to draw on.\033[0m\n\n";
      return;
   }

   if (cache.IsNull()) { // look next to this macro first, then in the usual workspace spot
      const char *rel[] = {"/plots/proton_kin_clean195.root", "/plots/proton_kin.root"};
      ...
      if (cache.IsNull() && gSystem->Getenv("VMCWORKDIR"))
         cache = TString(gSystem->Getenv("VMCWORKDIR")) +
                 "/macro/Unpack_HDF5/a2091/UKF/pp/plots/proton_kin_clean195.root";
      if (cache.IsNull()) {
         std::cerr << "\n\033[1;31m[explore_C15] no default cache found next to the macro (" << macroDir()
                   << "/plots/).\033[0m\n  Pass one explicitly: root -l 'explore_C15.C(\"/path/to/cache.root\")'\n\n";
         return;
      }
   }
   new C15Explorer(cache, mEjectAmu, mResidAmu, tag, Ebeam0);
}
