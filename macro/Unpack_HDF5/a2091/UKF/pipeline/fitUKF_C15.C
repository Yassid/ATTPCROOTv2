/// @file fitUKF_C15.C
/// @brief Run ONLY the new UKF fitter on a pre-reconstructed a2091 15C(p,p') file.
///
/// Ported from a1975 UKF/pipeline/fitUKF_a1975.C. Reads <fileName>_reco.root
/// (AtPatternEvent, from unpackReco_C15.C) and runs EventFit::AtFitterUKF wrapped
...
///
/// gasDensity default 3.308e-5 g/cm^3 = H2 at 300 torr, ROOM TEMPERATURE. This is the SAME
/// value carried by the H_300torr_RT medium of ATTPC_H300torr_RT (the geometry GENFIT uses),
/// so the UKF (CATIMA density) and GENFIT-with-material-effects passes see identical material
/// and can be compared apples-to-apples -- which is the whole point of running both fitters.
/// The 14C port used 3.553e-5 (a slightly colder gas assumption); if the a2091 g.s. sits off
/// zero after the beam-energy calibration, the gas TEMPERATURE is the first knob to revisit.
/// << CONFIRM density/temperature against the real a2091 run conditions. >>
///
///   root -b -q 'fitUKF_C15.C("run_0138", -1, "proton", -1)'
                                                      ...
                                                      double refInflation = 4.0)
{
   ...
   if (it == kParticleTable.end()) {
      std::cout << "\033[1;31m[fitUKF_C15] Unknown particle '" << name << "' — skipping.\033[0m" << std::endl;
      return nullptr;
   }
...
void fitUKF_C15(TString fileName = "run_0138", Long64_t nEvents = -1, TString particles = "proton",
                 Int_t bFieldSign = -1, Double_t bFieldMag = 2.85, Double_t gasDensity = 3.308e-5,
                 TString outSuffix = "",
                 TString ioDir = "/home/yassid/a2091_C15_reco/", Double_t measSigma = 0.5, Double_t momSigmaFrac = 0.1,
                 ...
                 Double_t refInflation = 4.0)
{
   ...
   if (gSystem->AccessPathName(inputFile.Data())) {
      std::cout << "\033[1;31mERROR: " << inputFile << " not found. Run unpackReco_C15.C first.\033[0m" << std::endl;
      return;
   }

   std::cout << "\033[1;33m=== fitUKF_C15 ===\033[0m" << std::endl;
