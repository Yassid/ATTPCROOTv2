#!/bin/bash
# GENFIT pass over the ALREADY-GATED (p,d) inputs written by fitpipe_pd_C15.sh, so the
...
one(){ local r="$1"; local L="$LOG/${r}_genfit.log"
  ...
  root -b -q -l "$HERE/pipeline/fitGenfit_C15.C(\"$r\",-1,\"$IN\",\"\",\"$OUTDIR\",-2.85,2,5,\"\",4.0,5.0,170.0,kTRUE,kFALSE,\"deuteron\")" > "$L" 2>&1
