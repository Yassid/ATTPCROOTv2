...
#   ./fitpipe_C15.sh "run_0138 run_0056 ..." 4
#
# Gas density and geometry are overridable so the same data can be refitted with
# different material. The a2091 gas is 300 torr H2 at room temperature (rho = 3.308e-5 g/cm3,
# matching the H_300torr_RT medium of ATTPC_H300torr_RT). Override to sweep the assumption:
#   DENSITY=3.553e-5 GEONAME=ATTPC_H300torr FITDIR=~/a2091_C15_fit_cold/ ./fitpipe_C15.sh "..." 4
...
FITDIR="${FITDIR:-/home/yassid/a2091_C15_fit/}"; IN="${FITDIR}in/"; LOG="${FITDIR}logs"; mkdir -p "$IN" "$LOG"
...
one(){ local r="$1"; local L="$LOG/${r}.log"
  ...
  # 1) gate
  root -b -q -l "$HERE/pipeline/gate_events_C15.C(\"$r\",\"$SLIM\",\"$IN\",\"$FREF${r}_reco.root\")" > "$L" 2>&1
  [ -f "$IN/${r}_reco.root" ] || { echo "gate failed $r"; return; }
  # 2) UKF
  root -b -q -l "$HERE/pipeline/fitUKF_C15.C(\"$r\",-1,\"proton\",-1,2.85,$DENSITY,\"\",\"$IN\",0.5,0.1,1,10,\"$FITDIR\")" >> "$L" 2>&1
  ...
  root -b -q -l "$HERE/pipeline/fitGenfit_C15.C(\"$r\",-1,\"$IN\",\"\",\"$FITDIR\",-2.85,2,5,\"\",4.0,10.0,170.0,kFALSE,kFALSE,\"proton\",\"$GEONAME\")" >> "$L" 2>&1
