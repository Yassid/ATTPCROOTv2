#!/bin/bash
# Chase the reco batch: as soon as a run is reported "done" in the reco batch log, slim it and
# run the full fit pass (gate -> UKF -> GENFIT) on it. Lets the whole 15C production finish
# unattended while reco is still chewing through the big runs.
#   ./follow_C15.sh "run_0065 run_0063 ..."
RUNS="${1:-}"
HERE="$(cd "$(dirname "$0")" && pwd)"
BATCHLOG=/home/yassid/a2091_C15_reco/batch.log
LOG=/home/yassid/a2091_C15_fit/follow.log
mkdir -p /home/yassid/a2091_C15_fit
# Only trust "done" lines appended AFTER we start. A killed batch leaves stale
# "done run_XXXX exit 15" lines behind; without this offset the follower matches
# them instantly, finds no reco file, and skips every run (hit 2026-07-26).
OFF=$(wc -l < "$BATCHLOG" 2>/dev/null || echo 0)
echo "$(date +%H:%M:%S) follow start, batch.log offset=$OFF, runs: $RUNS" >> "$LOG"
for r in $RUNS; do
  # wait for reco to report this run done (and the file to stop growing)
  while ! tail -n +$((OFF + 1)) "$BATCHLOG" 2>/dev/null | grep -q "done $r "; do sleep 30; done
  [ -f "/home/yassid/a2091_C15_reco/${r}_reco.root" ] || { echo "$(date +%H:%M:%S) $r: no reco file" >> "$LOG"; continue; }
  [ -f "/home/yassid/a2091_C15_fit/${r}_genfit.root" ] && { echo "$(date +%H:%M:%S) $r: already fitted" >> "$LOG"; continue; }
  echo "$(date +%H:%M:%S) $r: slim+fit start" >> "$LOG"
  "$HERE/slim_cache_batch.sh" "$r" 1 >> "$LOG" 2>&1
  "$HERE/fitpipe_C15.sh" "$r" 2   >> "$LOG" 2>&1
  echo "$(date +%H:%M:%S) $r: done" >> "$LOG"
done
echo "$(date +%H:%M:%S) FOLLOW DONE" >> "$LOG"
