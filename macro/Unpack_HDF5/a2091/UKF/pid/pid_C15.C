/// @file pid_C15.C
/// @brief PID plane (sqrt(dEdx) vs Brho) for a2091 15C(p,p') from the faithful Spyral
...
///
///   root -b -q 'pid/pid_C15.C("run_0138")'
///   root -b -q 'pid/pid_C15.C("run_0138,run_0058,run_0060", "/home/yassid/a2091_C15_reco/")'
...
void pid_C15(TString runsCSV = "run_0138", TString inDir = "/home/yassid/a2091_C15_reco/", Long64_t nEvents = -1,
              ...
              double sqrtMax = 80, double brhoMax = 2.5, double dedxMax = 4000)
{
   ...
   TH2F *hS = new TH2F("hS", "15C Spyral PID (red = 16C+p proton band);#sqrt{dEdx};B#rho [T m]", 400, 0, sqrtMax, 400,
                       0, brhoMax);
   TH2F *hSd = new TH2F("hSd", "15C Spyral PID;dEdx [counts];B#rho [T m]", 400, 0, dedxMax, 400, 0, brhoMax);
   TH2F *hKt = new TH2F("hKt", "dEdx vs #theta_{lab};#theta_{lab} [deg];dEdx [counts]", 180, 0, 180, 400, 0, dedxMax);

   TFile *fo = new TFile((plotDir + "pid_C15" + outTag + ".root").Data(), "RECREATE");
   ...
   c->cd(3);
   hKt->Draw("colz");
   TString png = plotDir + "pid_C15" + outTag + ".png";
   c->SaveAs(png);
   printf("saved %s\n", png.Data());
   fo->Close();
}
