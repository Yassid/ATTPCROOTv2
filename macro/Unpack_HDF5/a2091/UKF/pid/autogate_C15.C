...
///        working ROOT GUI here). Reads pid_C15_data.csv written by dump_pid_C15.C and
///        writes an AtParticleID JSON gate readable by gate_events_C15.C.
///
...
///
///   root -b -q 'pid/autogate_C15.C("proton_15C",1,1)'
///   root -b -q 'pid/autogate_C15.C("deuteron_15C",1,2,0.06,1.0,1.6,15,"","",2.0)'   // shifted band
void autogate_C15(TString name = "proton_15C", int Z = 1, int A = 1, double brLo = 0.06, double brHi = 1.05,
                  ...
                  double ridgeScale = 1.0, double sdMax = 60)
{
   ...
   if (csvFile.IsNull())
      csvFile = dir + "/pid_C15_data.csv";
   if (outJson.IsNull())
      outJson = dir + "/" + name + ".json";
   if (gSystem->AccessPathName(csvFile)) {
      printf("\033[1;31mERROR: %s not found (run dump_pid_C15.C first)\033[0m\n", csvFile.Data());
      return;
   }
   ...
   TH2F *h = new TH2F("h", TString::Format("a2091 15C PID with the auto %s gate;#sqrt{dEdx};B#rho [T m]", name.Data()),
