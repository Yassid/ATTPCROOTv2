...
///
/// Panels: (1) #IC-peaks distribution (events whose max is in the 15C window)
///         (2) Bp-vs-theta, mult==1 AND single peak in [icLo,icHi]  (CLEAN 15C)
...
///
///   root -b -q 'pid/ic_multiplicity_C15.C("run_0138,run_0058,run_0060,run_0061,run_0064")'
...
void ic_multiplicity_C15(TString runsCSV, TString inDir = "/home/yassid/a2091_C15_reco_slim/", double icLo = 500,
                          double icHi = 900, double peakThr = 200, Int_t tbLo = 800, Int_t tbHi = 1500,
                          double bField = 2.85,
                          TString protonGate = "/home/yassid/fair_install/ATTPCROOTv2/macro/Unpack_HDF5/"
                                               "a2091/UKF/pid/proton_15C.json")
{
   ...
   TH1D *hN = new TH1D("hN", "IC pulse multiplicity (max in 15C window);# IC pulses;events", 8, 0.5, 8.5);
   TH2F *b1 = new TH2F("b1", "B#rho vs #theta  MULT==1 (clean 15C);#theta_{lab} [deg];B#rho [T m]", 180, 0, 180, 300, 0, 1.5);
   ...
   c->cd(3); b2->Draw("colz");
   c->cd(4); b0->Draw("colz");
   TString png=plotDir+"ic_multiplicity_C15.png"; c->SaveAs(png); printf("saved %s\n",png.Data());
}
