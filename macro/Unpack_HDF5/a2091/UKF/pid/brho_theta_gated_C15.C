/// @file brho_theta_gated_C15.C
/// @brief 15C Bρ vs θ_lab with the IC beam gate (15C) AND the proton PID gate applied.
...
///
///   root -b -q 'pid/brho_theta_gated_C15.C("run_0138,run_0058", "/home/yassid/a2091_C15_reco_slim/")'
...
void brho_theta_gated_C15(TString runsCSV = "run_0138", TString inDir = "/home/yassid/a2091_C15_reco_slim/",
                           double icLo = 500, double icHi = 900, Int_t icTbLo = 1050, Int_t icTbHi = 1250,
                           double bField = 2.85, double arclenMin = 0, TString outTag = "",
                           TString protonGate =
                              "/home/yassid/fair_install/ATTPCROOTv2/macro/Unpack_HDF5/a2091/UKF/pid/"
                              "proton_15C.json")
{
   ...
   TH1D *hIC = new TH1D("hIC", "IC amplitude (red = 15C gate);ic amplitude;events", 500, 0, 2500);
   TH2F *hPID = new TH2F("hPID", "15C PID, IC-gated (red = proton gate);#sqrt{dEdx};B#rho [T m]", 400, 0, 45, 400, 0, 1.2);
   TH2F *hBt = new TH2F("hBt", "15C  B#rho vs #theta_{lab}  (IC + proton gated);#theta_{lab} [deg];B#rho [T m]", 180,
                        0, 180, 300, 0, 1.5);
   ...
   printf("\n== gated Brho-theta ==  matched=%ld  IC-15C=%ld (%.1f%%)  IC-gated tracks=%ld  +proton=%ld (%.1f%%)\n",
   ...
   TString png = plotDir + "brho_theta_gated_C15" + outTag + ".png";
   c->SaveAs(png);
   printf("saved %s\n", png.Data());
}
