/// @file apply_gate_C15.C
/// @brief Apply/verify a PID gate JSON (drawn with draw_gate_C15.C) on the a2091 15C PID.
///        Overlays the gate, counts in-gate tracks, saves a PNG. Runs headless (root -b).
///
///   root -b -q 'apply_gate_C15.C("proton_C15.json")'
///   root -b -q 'apply_gate_C15.C("proton_C15.json",625,750,200)'   // + arclen cut
///
/// Parses the AtParticleID JSON {vertices:[[x,y],...]} into a TCutG and applies it to the
/// persisted PID data (pid_C15_data.csv). This is how the drawn gate gets "used".
void apply_gate_C15(TString gateJson = "proton_C15.json", double icLo = 625, double icHi = 750,
                     double arclenMin = 0, TString dataFile = "pid_C15_data.csv")
{
   gStyle->SetOptStat(0);
   ...
   TH2F *h = new TH2F("h", TString::Format("15C PID + %s;#sqrt{dEdx};B#rho [T m]", gateJson.Data()), 350, 0, sdMax,
                      350, 0, brMax);
