#!/bin/bash
# GENFIT pass over the ALREADY-GATED 15C inputs, with MATERIAL EFFECTS ON.
#
# This is the configuration that fixed 12Be (genfit_pd_Be12.sh, after the AtGenfitter
# dE/dx fix 5f87e625): matEffects=kTRUE. The 15C production never got it -- fitpipe_C15.sh
...
one(){ local r="$1"; local L="$LOG/${r}_genfit.log"
  ...
  root -b -q -l "$HERE/pipeline/fitGenfit_C15.C(\"$r\",-1,\"$IN\",\"\",\"$OUTDIR\",-2.85,2,5,\"\",4.0,10.0,170.0,kTRUE,kFALSE,\"proton\",\"$GEONAME\")" > "$L" 2>&1
