...
#
# NOTHING IS OVERWRITTEN. Output goes to reco_pt_catima/ with suffix _ptcat, alongside both
...
export OUT=/mnt/f/a1975/reco_pt_catima/
export LOG=/mnt/f/a1975/logs_pt_catima/
export GEO=ATTPC_H300torr_RT
export RHO=3.308e-5
export TAB=triton_H2_catima.txt
export GATE=/home/yassid/a1975_C16dt_analysis/triton_pt.json
# NATIVE CATIMA dE/dx, added to the GenFit fork on 2026-08-19. The (p,d) script this was copied
# from predates that and still runs on the ASCII table; there is no reason to inherit the gap.
#
# For THIS channel it should change little either way. Native CATIMA replaces the table only below
# beta*gamma = 0.05, which is KE = 3.5 MeV for a triton, and these tritons are 30-80 MeV -- almost
# the whole sample sits in Bethe-Bloch territory. The table stays loaded as the per-step fallback.
...
one() {
  ...
  fo="${OUT}${run}_genfitter_ptcat.root"
  [ -s "${fo}.done" ] && { echo "[have] $run"; return 0; }
  root -l -b -q "pipeline/fitGenfitter_a1975.C(\"${run}\",-1,\"$RECO\",\"_ptcat\",\"$OUT\",\
-2.85,2,5,\"$TAB\",\"$GATE\",4.0,kTRUE,1000010030,3.01604928,1,kFALSE,\"$GEO\",kTRUE,kTRUE,kFALSE,${RHO},kTRUE,${CEL},${CFULL})" \
    > "${LOG}gf_${run}.log" 2>&1
...
echo "=== (p,t) CATIMA production: $(echo $RUNS | wc -w) runs, $NPAR parallel -> $OUT ==="
echo "=== geometry $GEO (rho $RHO g/cm3), matFX ON, CATIMA MSC+straggling, fallback OFF ==="
printf '%s\n' $RUNS | xargs -P "$NPAR" -I{} bash -c 'one "$@"' _ {}
echo "=== done: $(ls "$OUT"/*_genfitter_ptcat.root 2>/dev/null | wc -l) fit files in $OUT ==="
