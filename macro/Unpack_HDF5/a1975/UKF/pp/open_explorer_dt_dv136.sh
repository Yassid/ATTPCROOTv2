...
root -b -l -q "$HERE/mkexp_pp.C(\"$CMAT\",\"$TMP/exp_dt_dv136.root\",1e9,900,1300)"
root -b -l -q "$HERE/mkexp_pp.C(\"$CCAT\",\"$TMP/exp_dt_dv125.root\",1e9,900,1300)"
if [[ "$PRIMARY" == "dv 1.25" ]]; then
  FIRST="$TMP/exp_dt_dv125.root"; SECOND="$TMP/exp_dt_dv136.root"; L1="dv 1.25"; L2="dv 1.136"
else
  FIRST="$TMP/exp_dt_dv136.root"; SECOND="$TMP/exp_dt_dv125.root"; L1="dv 1.136"; L2="dv 1.25"
fi
root -b -l -q "$MK(\"$FIRST\",\"$OUT\",\"16C(d,t)15C  dv 1.136 vs 1.25\",$EBEAM,16.0147013,2.0135532,3.01550072,15.0105993,16,\"0:g.s.,0.740:5/2+,1.218:Sn,3.103,4.220,4.657\",\"$SECOND\")"
