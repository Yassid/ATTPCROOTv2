...
void pid_pd(int runLo=106, int runHi=113, TString dir="/mnt/f/a1975/reco_pd/",
            ...
            TString out="/tmp/pid_pd.png")
{
   gSystem->Load("libAtReconstruction.so");
   gStyle->SetOptStat(0); gStyle->SetPalette(kBird); gStyle->SetNumberContours(255);

   auto*h=new TH2F("hpid","^{16}C+p Spyral PID;sqrt(dE/dx)  [arb];B#rho  [T#upointm]",300,0,30,250,0,2.5);
   long ntot=0;
   ...
   printf("filled %ld valid PID points from runs %d-%d\n",ntot,runLo,runHi);
