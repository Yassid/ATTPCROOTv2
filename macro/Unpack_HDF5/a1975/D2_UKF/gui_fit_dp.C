...
///
/// The fitter configuration reproduces prod_dp_catima.sh EXACTLY at the default settings, and
/// that is checked, not assumed: with the defaults, run_0020 entry 9789 must return
/// KE 0.385 MeV, theta 96.92 deg, chi2/ndf 143.66. The banner prints a warning if it does not.
/// (Omitting SetELossHybrid alone moved that to KE 4.517 -- one missing setter makes this a
/// different fitter, and then nothing it shows means anything.)
///
///   root -l 'gui_fit_dp.C("run_0020_multifit",9789,0)'
///   setsid bash -c 'tail -f /dev/null | root -l "gui_fit_dp.C(\"run_0020_multifit\",9789,0)"' &
///
...
class DpFitGui {
public:
   DpFitGui(TString runTag, Long64_t entry, int tid, TString gfDir, TString listFile)
      : fRunTag(runTag), fEntry(entry), fTid(tid), fGfDir(gfDir)
   {
      TString dir = gSystem->Getenv("VMCWORKDIR");
      { TFile *gf = TFile::Open(dir + "/geometry/ATTPC_D300torr_v2_geomanager.root"); gf->Get("FAIRGeom"); }
      fEloss = dir + "/macro/Unpack_HDF5/a1975/D2_UKF/proton_D2_300torr.txt";
   ...
   {
      fClusters.clear();
      TString fn = fGfDir + fRunTag + "_genfitter_p.root";
   ...
   void Refit()
   {
      ...
      fitter->SetELossHybrid(kTRUE, 6.61e-5);      // the production does this whenever a table is given
...
private:
   void MakeGui()
   {
      ...
      main->SetWindowName("a1975 16C(d,p)17C  --  live single-track genfit refit");
      ...
      fLog->AddLine("CONTROL: run_0020 entry 9789 with the defaults must give KE 0.385, theta 96.92, chi2/ndf 143.66.");
...
void gui_fit_dp(TString runTag = "run_0020_multifit", Long64_t entry = 9789, int tid = 0,
                TString gfDir = "/mnt/f/a1975/gf_dp_cateloss/",
                TString listFile = "/mnt/f/a1975/caches/.explorer/refit_list.txt")
{
   gSystem->Load("libAtReconstruction.so");
   gSystem->Load("libAtTools.so");
   FairLogger::GetLogger()->SetLogScreenLevel("ERROR");
   gStyle->SetOptStat(0);
   new DpFitGui(runTag, entry, tid, gfDir, listFile);
