...
static std::unique_ptr<EventFit::AtGenfitter> voFitter(TString eloss)
{
   ...
   // *** PRODUCTION CONFIGURATION ***  (trunc_scan_dp.C uses the OPPOSITE of both)
   // production/gui default: AtGenfitter's own z sort, and backwardSeedFix ON so a PRA-backward
   // track is seeded from the far end of that sort. Every truncation number measured with
   // vertex_order_fit_dp's voFitter describes the OTHER configuration and does NOT reproduce what
   // production, or the GUI at its default mode, actually does.
   f->SetUseClusterOrder(kFALSE);
...
void trunc_scan_prod_dp(TString runTag="run_0020_multifit", Long64_t entry=9789, int tid=0,
                         ...
                         TString eLossTable="proton_D2_300torr.txt")
{
   ...
   if (csvOut.Length()) { csv.open(csvOut.Data());
      csv << "run,entry,tid,ncl,geotheta,fromEnd,pct,nUsed,ok,ke,theta,chi2ndf,vxy,vz,prod_ke,prod_theta,prod_chi2\n"; }
   ...
   for (auto &j : jobs) {
      ...
      // --- FIXED-FRACTION SCAN (this file's only difference from vertex_order_fit_dp) -------
      // No chi2 threshold anywhere: fit a prefix of EVERY fraction from both ends and record all
      // of them. chi2 falls monotonically as clusters are removed, so it cannot referee its own
      // truncation -- that is what produced the 13%-biased KE when "best chi2" chose the cut.
      // Emitting the whole curve lets the cut point be read off a KE PLATEAU instead.
      for (int fromEnd = 0; fromEnd <= 1; ++fromEnd) {
         for (int pct = 100; pct >= minPct; pct -= stepPct) {
            ...
            if (csvOut.Length())
               csv << rt << "," << en << "," << id << "," << all.size() << "," << geoTh << ","
                   << fromEnd << "," << pct << "," << o.nUsed << "," << (o.ok ? 1 : 0) << ","
                   << o.ke << "," << o.th << "," << o.c2n << "," << o.vxy << "," << o.vz << ","
                   << prod.ke << "," << prod.th << "," << prod.c2n << "\n";
         }
      }
      if (++done % 10 == 0) printf("  %ld / %zu\n", done, jobs.size());
   }
   if (f) f->Close();
