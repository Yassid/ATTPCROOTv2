...
void ex_dp_a1975(TString runsCSV = "run_0016", TString inDir = "/mnt/f/a1975/reco_d2/",
                 ...
                 double chi2Cut = 10.0, double exShift = 0.0, double keMax = 50.0)
{
   ...
   TFile *fcache = new TFile("proton_kin_dp.root", "RECREATE");
   TNtuple *ntk =
      new TNtuple("pk", "candidate proton kinematics (d,p)", "ke:theta:vertexz:thcm:ex:chi2ndf:brho:dedx:sqrtdedx");

   TH1F *hex = new TH1F(
      "hex", "^{17}C excitation energy (d,p), proton hyp;E_{x}(^{17}C) [MeV];proton candidates", 200, -10, 20);
   TH2F *hexcm = new TH2F("hexcm", "E_{x}(^{17}C) vs #theta_{cm};#theta_{cm} [deg];E_{x} [MeV]", 180, 0, 180, 150, -10,
                          20);
   ...
   fcache->cd();
   ntk->Write();
   printf("cached -> proton_kin_dp.root (ntuple pk: ke:theta:vertexz:thcm:ex:chi2ndf:brho:dedx:sqrtdedx)\n");
   ...
   c->cd(4);
   hpid->Draw("colz");
   c->SaveAs("plots/ex_dp_spectrum.png");
   printf("saved plots/ex_dp_spectrum.png  (red dashed line in panel 1 = 17C g.s. Ex=0)\n");
}
