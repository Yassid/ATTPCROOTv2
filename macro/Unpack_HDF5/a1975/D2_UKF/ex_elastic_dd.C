...
///        16C(d,d)17C neutron-stripping channel (a1975 D2-target runs 0016-0103).
///
...
/// m3=p, m4=17C reconstructs the 16C excitation energy. A genuine (d,d) stripping
...
void ex_elastic_dd(TString runsCSV = "run_0016", TString inDir = "/mnt/f/a1975/reco_d2/",
                 TString fitSuffix = "", TString gateFile = "pid/deuteron_band_d2.json", double Ebeam = 192.0,
                 double chi2Cut = 10.0, double exShift = 0.0, double keMax = 50.0)
{
   ...
   TFile *fcache = new TFile("deuteron_kin_dd.root", "RECREATE");
   TNtuple *ntk =
      new TNtuple("pk", "candidate proton kinematics (d,d)", "ke:theta:vertexz:thcm:ex:chi2ndf:brho:dedx:sqrtdedx");

   TH1F *hex = new TH1F(
      "hex", "^{16}C excitation energy (d,d), proton hyp;E_{x}(^{16}C) [MeV];proton candidates", 200, -10, 20);
   ...
   for (int ri = 0; ri < runs->GetEntries(); ++ri) {
      ...
      TString gf = inDir + run + "_genfitter_d" + fitSuffix + ".root";
      ...
      for (Long64_t i = 0; i < N; ++i) {
         ...
         for (auto &sr : pidev->GetSpyral()) {
            ...
            if (thDegCut < 10.0 || thDegCut > 170.0) // drop near-beam forward/backward
               continue;
            auto v = ft->GetVertex();
            auto [ex, thcm] = kine_2b(m_C16, m_d, m_d, m_C16, Ebeam, thRad, ke);
            if (std::isnan(ex))
               continue;
            ++nCand;
   ...
   printf("\nvalid Spyral PID: %ld -> in proton gate: %ld -> proton candidates (d,d) -> 17C Ex: %ld\n", nValPID, nGated,
          nCand);
   fcache->cd();
   ntk->Write();
   printf("cached -> deuteron_kin_dd.root (ntuple pk: ke:theta:vertexz:thcm:ex:chi2ndf:brho:dedx:sqrtdedx)\n");
   ...
   c->cd(4);
   hpid->Draw("colz");
   c->SaveAs("plots/ex_elastic_dd.png");
   printf("saved plots/ex_elastic_dd.png  (red dashed line in panel 1 = 16C g.s. Ex=0)\n");
}
