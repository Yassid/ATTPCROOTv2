#!/usr/bin/env bash
# Build the (d,t) kinematics cache for the dv 1.10424 production fitted with genfit
# MATERIAL EFFECTS ON (gf_dt_maton). Identical to cache_dt_dv1104.sh in every selection --
# same gate, same Ebeam, no cuts -- so the two caches differ ONLY by the fitter's matFX
# setting and can be differenced directly.
#
...
#
#   inDir   /mnt/f/a1975/gf_dt_dv1104/, and the run string carries the _multifit tag because
...
NPAR="${1:-6}"
WORK="${2:-/mnt/f/a1975/caches/.maton}"
OUT=/mnt/f/a1975/caches/dt_kin_maton.root
FIT=/mnt/f/a1975/gf_dt_maton/
EBEAM=184.17
