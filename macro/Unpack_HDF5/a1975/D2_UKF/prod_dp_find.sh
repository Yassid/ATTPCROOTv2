...
export GF="${GF:-/mnt/f/a1975/gf_dp_find/}"
export LOG="${LOG:-/mnt/f/a1975/logs_dp_find/}"
...
\"ATTPC_D300torr_v2_geomanager.root\",kTRUE,0,2,\"$PAR\",kFALSE,kFALSE,kFALSE,\"$TAB\",kTRUE,kTRUE,${CEL},${CFULL},0,kTRUE,${C2MAX})" \
    > "${LOG}gf_${r}.log" 2>&1
...
echo "=== CATIMA (d,p) production WITH FIND (chi2/ndf < $C2MAX): $(echo $RUNS | wc -w) runs, $NPAR parallel -> $GF ==="
