...
#
# THIS IS prod_dp_catima.sh WITH THE Z SORT OFF (SetUseClusterOrder), FIND optional.
# AtGenfitter orders clusters by drift z, valid only while the helix advances in z faster than the
# z resolution. On a1975 (d,p) backward tracks it often does not -- 1.42 mm per cluster against
# 1.84 mm for one time bucket -- so the sort orders NOISE. run_0016 e11488: 166.8 MeV z-sorted
# against 5.39 MeV from its own pattern circle; keeping the cluster order gives 5.96.
# KNOWN NOT UNIFORMLY SAFE (2026-09-05, five tracks): one lost its fit, one swung theta 100 deg.
# (original FIND header follows) Each track is fitted at full
...
export GF="${GF:-/mnt/f/a1975/gf_dp_clord/}"
export LOG="${LOG:-/mnt/f/a1975/logs_dp_clord/}"
...
# FIND on top of the cluster order, or not. Default kFALSE so the FIRST question asked is the
# single-variable one: what does dropping AtGenfitter's z sort do on its own?
export FIND="${FIND:-kFALSE}"
...
\"ATTPC_D300torr_v2_geomanager.root\",kTRUE,0,2,\"$PAR\",kFALSE,kFALSE,kFALSE,\"$TAB\",kTRUE,kTRUE,${CEL},${CFULL},0,${FIND},${C2MAX},kTRUE)" \
    > "${LOG}gf_${r}.log" 2>&1
...
echo "=== (d,p) production: z-sort OFF (cluster order kept), FIND=$FIND, C2MAX=$C2MAX -- $(echo $RUNS | wc -w) runs, $NPAR parallel -> $GF ==="
