/// @file gate_draw_dp.C
/// @brief Buttoned gate drawer for the a1975 16C(d,p)17C PROTON gate. Ported 2026-09-05 from the
///        CURRENT drawer, C15p/pid/gate_draw_C15p.C (itself from the a1954 14C(p,t) one) -- same
///        GUI, this experiment's plane, beam, target and kinematics.
///
/// Reads pid/pid_plane_dt_dv1104.root, built by pid/pid_plane_dt.C from the PATTERN tracks, so the
/// gate can be drawn before anything is fitted -- which matters, because the fit needs the gate.
///
/// ★ THE "dt" IN THE CACHE NAME IS NOT A MISTAKE AND IT IS NOT A (d,t) PLANE. pid_plane_dt.C runs
/// AtSpyralPID::Estimate on every pattern track with NO species hypothesis and NO cuts (IC is
/// stored, never applied), so ONE plane serves every D2 ejectile. 135778 tracks over all 47 runs.
/// Do not rebuild it to draw a proton gate; draw on this one, as the triton gate was.
///
/// ★ THIS PLANE IS NOT GAIN MATCHED. The C15p original carries a warning that its points file has
/// the per-run gain table already applied; a1975's does not, and pid_plane_dt.C applies no such
/// correction. So a per-run gain drift shows up here as a WIDENED band rather than as a shifted
/// one, and it has not been measured on this run set. Do not copy C15p's "already gain matched"
/// claim into anything downstream.
///
...
///   root -l 'pid/run_gate_dp.C'      // detached-safe launcher, keeps the window open
///   root -l 'pid/gate_draw_dp.C'     // from an interactive prompt
///
/// ★ THE BEAM ENERGY IS CONTESTED AND THE LOCUS DEPENDS ON IT. The same 47 D2 runs are carried as
/// 184.25 MeV (working_point.sh A1975_DT_EBEAM, measured on these runs and adopted for (d,t)),
/// 188.33 (C17_dp_fits, what the whole existing (d,p) analysis used) and 192.0 (ex_dp_a1975.C's
/// default). The default here is 184.25. Unlike the C15p original -- which defaults eBeam to 0 to
/// DISABLE the locus, because its beam energy was never established -- a1975 has a measured value,
/// so the locus is available; but if the red dots sit off the ridge, try 188.33 before moving the
/// polygon. Do not treat the curve as ground truth while that disagreement is open.
///
/// ★ IC DEFAULTS TO [900,1300]. The D2 beam is a cocktail -- 16C near 1150 and a second component
/// near 2060 in comparable numbers -- so an ungated plane is two beams at once. Pass icLo < 0 to
/// see the raw plane.
///
/// Default channel is 16C(d,p)17C -- the proton ejectile. Z/A select another via the table below:
///   (d,d) elastic : Z 1 A 2        (d,t) 15C : Z 1 A 3   <- the adopted a1975 channel, a cross-check
#include <vector>
...
static double dpBrho16Cdp(double thlab, double Eb = 184.25, double exStar = 0.0,
                          double m3Amu = 1.00782503207, double m4Amu = 17.022579)
{
   const double u = 931.49401;
   const double m1 = 16.0147013 * u;  // 16C beam
   const double m2 = 2.0135532 * u;   // DEUTERIUM target, NUCLEAR deuteron mass (NOT the atomic
                                      // 2.01410178). THE CONVENTION IN THIS FILE, and it is not a
                                      // free choice: m1 and m4 are ATOMIC (6 electrons each side
                                      // for a carbon), so the light m2/m3 must be NUCLEAR for the
                                      // electrons to cancel. Mixing them is a ~0.5 MeV error per
                                      // electron that looks exactly like a calibration offset.
                                      // Verified: with this convention elastic 16C(d,d)16C gives
                                      // Q = 0.0000 exactly, and (d,p) gives Q = -1.498 MeV against
                                      // Sn(17C) - B(d) = 0.729 - 2.225 = -1.496.
   const double m3 = m3Amu * u;       // ejectile
   const double m4 = m4Amu * u + exStar;      // residual
   // EXACT solution, replacing the inherited fixed-step scan. TWO reasons, both measured on this
   // channel rather than assumed:
   //   1. THE SCAN DROPPED OUT. It accepted the first ke with |m4(ke) - m4| < 0.06 MeV on a 0.05
   //      MeV grid. For the SLOW backward protons of (d,p), dm4/dke is steep enough that a step
   //      jumps the window: 10 of the 90 degrees from 90-179 returned -1, i.e. holes in the red
   //      overlay exactly where this channel's locus is read. (d,t) never saw it -- its triton is
   //      forward and fast.
   //   2. IT WAS TOO SLOW TO LEAVE ON. This file is loaded INTERPRETED (.L, no ACLiC) and the
   //      drawer calls this once per track; ~3400 iterations x 76k tracks reads as a hang, which
   //      is why the C15p original ships showLocus = false. Exact makes it free, so it can default
   //      to ON -- and for (d,p) the locus is the only thing that says which band is the proton.
   //
   // Conservation, with E = E1 + m2, P = p_beam, s = E^2 - P^2, K = s + m3^2 - m4^2:
   //     2 E sqrt(p3^2 + m3^2) - 2 P cos(theta) p3 = K
   // Squaring once leaves a quadratic in p3. Squaring can invent a root, so every candidate is put
   // back into the UNSQUARED equation before it is accepted.
   double E1 = Eb + m1, P = std::sqrt(E1 * E1 - m1 * m1);
   double c = std::cos(thlab * TMath::DegToRad());
   double E = E1 + m2, ss = E * E - P * P, K = ss + m3 * m3 - m4 * m4;
   double A = 4.0 * (E * E - P * P * c * c), B = -4.0 * K * P * c, C = 4.0 * E * E * m3 * m3 - K * K;
   if (std::fabs(A) < 1e-9) return -1;
   double disc = B * B - 4.0 * A * C;
   if (disc < 0) return -1; // no kinematic solution at this angle
   double sq = std::sqrt(disc), best = -1;
   double roots[2] = {(-B - sq) / (2 * A), (-B + sq) / (2 * A)};
   for (int i = 0; i < 2; ++i) {
      double p3 = roots[i];
      if (p3 <= 0) continue;
      double E3 = std::sqrt(p3 * p3 + m3 * m3);
      if (std::fabs(2 * E * E3 - 2 * P * c * p3 - K) > 1e-3 * std::fabs(K)) continue; // spurious
      // WHICH ROOT: the UPPER one. Two-body inverse kinematics is DOUBLE-VALUED at forward
      // angles for a heavy ejectile, and the inherited scan swept ke upward with NO break, so
      // `best = ke` kept the LAST match -- the higher branch. Taking the lower one instead
      // disagrees by up to 100%: measured against the scan over 1-179 deg, max fractional
      // difference 0.91 for (d,t) and 0.99 for (d,d), while (d,p) barely noticed (5.5e-3)
      // because its proton locus is single-valued over almost the whole range. So this choice
      // is invisible on the channel being drawn and glaring on the cross-check channels --
      // exactly the kind of thing that would have shipped unnoticed.
      if (p3 > best) best = p3;
   }
   if (best < 0) return -1;
   return best / 299.792458;
}
                            ...
                            double m3Amu = 1.00782503207, double m4Amu = 17.022579)
{
   ...
   for (double th = 1; th <= 179; th += 1.0) {
      double b = dpBrho16Cdp(th, Eb, exStar, m3Amu, m4Amu);
      if (b <= 0) continue;
...
/// band against ITS OWN channel. Everything below hangs off one fact: the compound here is
/// 16C + d, so Z = 6+1 = 7 and A = 16+2 = 18, and fixing the ejectile fixes the residual.
///     ejectile p (1,1) -> 17C      d (1,2) -> 16C      t (1,3) -> 15C
///              3He (2,3) -> 15B    alpha (2,4) -> 14B
/// (d,t) is the channel with an adopted analysis, so its row is the cross-check: opening the
/// drawer at Z=1 A=3 must reproduce the band triton_d2_dv1104.json was drawn on.
/// Excitation energy of the second, dashed guide locus. 2.763 MeV = the lowest 17C resonance.
static const double kExGuide = 2.763;

static void dpChannelMasses(int Z, int A, double &m3Amu, double &m4Amu)
{
   struct E { int Z, A; double m3, m4; };
   static const E tab[] = {
      // m3 NUCLEAR, m4 ATOMIC -- see the convention note in dpBrho16Cdp. Only the (d,t) row was
      // right before 2026-09-05: it came from the working (d,t) code, the others were filled from
      // an atomic-mass table and each carried a spurious +Z electron masses in the Q-value
      // (0.511 MeV for p and d, 1.022 for 3He and alpha). The (d,p) locus this file exists to draw
      // was therefore low by 1.6% in Brho at 60 deg and 4.6% at 150 deg -- worst exactly where the
      // transfer protons are. Each row is checked against its known Q below.
      {1, 1, 1.00727646688, 17.0225864},  // (d,p)   -> 17C   THE CHANNEL THIS FILE IS FOR  Q -1.498
      {1, 2, 2.01355321000, 16.0147013},  // (d,d')  -> 16C   elastic / inelastic           Q  0.000
      {1, 3, 3.01550072000, 15.0105993},  // (d,t)   -> 15C   the adopted a1975 channel     Q +2.007
      {2, 3, 3.01493216000, 15.0310300},  // (d,3He) -> 15B                                 Q -16.49
      {2, 4, 4.00150609000, 14.0254040},  // (d,a)   -> 14B                                 Q +1.252
   };
   for (const auto &e : tab)
      if (e.Z == Z && e.A == A) { m3Amu = e.m3; m4Amu = e.m4; return; }
   // Fall back to the compound A = 18 rather than C15p's 16, or an unlisted species silently
   // gets a residual from the wrong experiment.
   m3Amu = A;
   m4Amu = 18 - A;
}

class DpGateDraw : public TObject {
public:
   DpGateDraw(TString outJson, TString cache, TString refP, TString refD, double xMax, double yMax,
   ...
   {
      dpChannelMasses(fZ, fA, fM3, fM4);
      printf("channel: ejectile Z=%d A=%d (m3 %.6f u), residual m4 %.6f u\n", fZ, fA, fM3, fM4);

      fH = new TH2F("hpt",
                    Form("a1975 D2 (d,p) PID, dv 1.10424, %s  --  draw the gate"
   ...
   void LocusCheck()
   {
      ...
      gAlt->Draw("L same");
      gUse->Draw("L same");
      // A second locus at finite Ex, so the g.s. band can be told from its neighbours. 2.763 MeV is
      // the lowest 17C resonance in the C17_dp_fits level list; it was 3.368, a 12C level inherited
      // from the (p,t) drawer this file was ported from and meaningless against a 17C residual.
      auto *gEx = dpLocusGraph(fFlip, fEbeam, kExGuide, kMagenta + 1, fM3, fM4);
      gEx->SetLineStyle(7);
      gEx->Draw("L same");
      ...
      leg->AddEntry(gEx, Form("E_{x} = %.3f MeV", kExGuide), "l");
   ...
   void Save()
   {
      ...
      //
      // xaxis is "sqrtdedx" HERE, not C15p's "sqrt_dEdx". AtCut2D only stores the string, so
      // nothing switches on it -- which is exactly why it drifts. a1975's convention is
      // "sqrtdedx": it is the branch name in pid_plane_dt.C's cache and the spelling in both
      // triton_d2_dv1104.json (the gate the adopted (d,t) production actually ran with) and
      // proton_band_d2_v2.json. Writing C15p's spelling would make this the only a1975 gate that
   ...
   void SavePNG()
   {
      ...
      fCanvas->SaveAs("pid/plots/gate_draw_dp.png");
      printf("saved pid/plots/gate_draw_dp.png\n");
   }
   ...
   /// Show or hide the elastic 16C(d,d)16C deuteron band.
   void ToggleDeuteron()
   {
      fShowDeut = !fShowDeut;
      printf("elastic 16C(d,d) deuteron band %s\n", fShowDeut ? "ON (violet, dashed)" : "OFF");
      Redraw();
   }
   ...
   void Redraw()
   {
      ...
      // The ELASTIC deuteron band 16C(d,d)16C, drawn so the proton gate can be placed AWAY from it
      // rather than by eye. Deuterons are the natural contaminant here: same Z, so the same charge
      // state, and a rigidity that runs through the same region of the plane.
      // WAS (2.01410178, 14.0032420) until 2026-09-05 -- the 14C(p,d)13C band inherited verbatim
      // from the drawer this file was ported from, except that 14.0032420 is 14N, not 13C. It was
      // ON BY DEFAULT, so a meaningless curve was being drawn right where the gate gets placed.
      if (fShowDeut) {
         if (!fDeutLoc) fDeutLoc = dpLocusGraph(fFlip, fEbeam, 0.0, kViolet + 1, 2.01355321, 16.0147013);
   ...
   /// EXIT THE PROCESS, hard. It was gApplication->Terminate(0), and that is the bug that made
   /// this drawer respawn every time the window was closed: launched as `root -l run_gate_dp.C`,
   /// the launcher called gApplication->Run() from INSIDE the command-line macro TRint was already
   /// processing. Terminate() then merely unwound that nested Run(), TRint resumed its file list,
   /// re-executed the macro and popped a FRESH window -- so closing the window spawned another,
   /// without limit (measured: ~2 new windows/second). gSystem->Exit bypasses the unwind entirely
   /// and cannot be re-entered no matter how the macro is launched.
...
private:
   void Load(TString cache)
   {
      ...
      for (Long64_t i = 0; i < t->GetEntries(); ++i) {
         ...
         double b = dpBrho16Cdp(fFlip ? 180.0 - pol : pol, fEbeam, 0.0, fM3, fM4);
      ...
      if (fIcLo >= 0 && fIcHi > fIcLo && !hasNP)
         printf("\033[1;33mno npulse branch -- pile-up NOT removed, so this plane is wider than the "
                "one the genfit production will select on\033[0m\n");
      f->Close();
   }
   ...
   void MakeGui()
   {
      ...
      // The [X] button had no handler, so it destroyed the frame and left the process running with
      // no window -- an invisible ROOT holding the PID plane open. Route it to the same hard exit.
      main->Connect("CloseWindow()", "DpGateDraw", this, "Quit()");
      main->DontCallClose();
      ...
      main->SetWindowName(Form("a1975 (d,p) PID  --  draw the %s gate (Z=%d A=%d)",
                               (fZ == 1 && fA == 1)   ? "proton"
                               : (fZ == 1 && fA == 2) ? "deuteron"
                               : (fZ == 1 && fA == 3) ? "triton"
                               : (fZ == 2 && fA == 4) ? "alpha"
      ...
      for (int i = 0; i < 9; ++i) {
         ...
         b->Connect("Clicked()", "DpGateDraw", this, slot[i]);
      ...
      fEThLo->Resize(70, 22);
      fEThLo->Connect("ValueSet(Long_t)", "DpGateDraw", this, "ApplyAngle()");
      ...
      fEThHi = new TGNumberEntry(bar, 180.0, 5, -1, TGNumberFormat::kNESRealOne,
                                 ...
                                 TGNumberFormat::kNELLimitMinMax, 0.0, 180.0);
      fEThHi->Resize(70, 22);
      fEThHi->Connect("ValueSet(Long_t)", "DpGateDraw", this, "ApplyAngle()");
      bar->AddFrame(fEThHi, new TGLayoutHints(kLHintsLeft, 2, 4, 4, 4));
      {
         auto *ba = new TGTextButton(bar, "Apply angle");
         ba->Connect("Clicked()", "DpGateDraw", this, "ApplyAngle()");
      ...
      auto mkNum = [&](TGNumberEntry *&e, double v, double lo, double hi, int w, const char *lbl,
         ...
         e->Resize(w, 22);
         e->Connect("ValueSet(Long_t)", "DpGateDraw", this, "ApplyAxes()");
   ...
   // Overwritten by dpChannelMasses in the ctor, so these are only the pre-ctor value -- but they
   // were the ATOMIC proton against 16C (and 16.0147413 is a typo for 16.0147013), i.e. neither
   // the right convention nor the right residual. Kept in step with the (d,p) row of the table.
   double fM3 = 1.00727646688, fM4 = 17.0225864; ///< ejectile/residual for THIS species' locus
...
/// @param eBeam     only the locus depends on it, not the gate. 184.25 is working_point.sh's
///                  measured value for these 47 runs; 188.33 is what C17_dp_fits used. Unresolved.
/// @param flipPolar stored polar = 180 - theta_lab, which is pid_plane_dt.C's convention.
///                  Check it in the Locus check panel rather than trusting it.
/// @param yMax      1.6, not C15p's 2.0: the (d,p) proton band runs Brho 0.17 (backward) to ~1.05
///                  (forward), so 1.6 fills the pad with the population being cut on.
void gate_draw_dp(TString outJson = "pid/proton_d2_dv1104.json",
                       TString cache = "pid/pid_plane_dt_dv1104.root",
                       ...
                       int Z = 1, int A = 1)
{
   new DpGateDraw(outJson, cache, refP, refD, xMax, yMax, icLo, icHi, showLocus, eBeam, flipPolar, Z, A);
}
