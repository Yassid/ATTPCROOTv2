...
void list_ex_dp(TString runsCSV = "run_0016_multifit", TString inDir = "/mnt/f/a1975/gf_dp_cateloss/",
                ...
                double exLo = -1e9, double exHi = 1e9,
                TString listOut = "")
{
   ...
   std::ofstream exList;
   if (listOut.Length()) exList.open(listOut.Data());
   long nInWindow = 0;
      ...
      for (Long64_t i = 0; i < N; ++i) {
         ...
         for (auto &sr : pidev->GetSpyral()) {
            ...
            if (!inGate)
               continue;
            ++nCand;
            // GUI list: <run> <entry> <tid>, exactly the tracks this cache counted
            if (exList.is_open() && exCal >= exLo && exCal <= exHi) {
               TString rnum = run; rnum.ReplaceAll("run_", ""); rnum.ReplaceAll("_multifit", "");
               exList << rnum << " " << i << " " << sr.trackID << "\n";
               ++nInWindow;
            }
            auto v = ft->GetVertex();
