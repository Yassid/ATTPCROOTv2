/// @file ic_gated_pid_Be12.C
/// @brief PID plane (sqrt(dEdx) vs Brho) for a1954 12Be, with the IC BEAM GATE applied.
///        12Be beam = IC-amplitude peak ~625-750 (NOT the dominant ~1900 contaminant).
///
...
/// Fills the PID plane for IC-gated (12Be) vs all events; overlays the 16C+p proton band.
///
///   root -b -q 'ic_gated_pid_Be12.C("run_0142,run_0147,run_0148,run_0149,run_0150")'
void ic_gated_pid_Be12(TString runsCSV = "run_0142", TString inDir = "/home/yassid/a1954_Be12_reco/",
                          ...
                          "/home/yassid/fair_install/ATTPCROOTv2/macro/Unpack_HDF5/a1975/UKF/pid/proton_band.json")
{
   ...
   TH2F *hGate = new TH2F("hGate", "PID (12Be-beam gated, IC 625-750);#sqrt{dEdx};B#rho [T m]", 400, 0, 45, 400, 0,
                          1.0);
   TH1D *hIC = new TH1D("hIC", "IC amplitude (red = 12Be gate);ic_amplitude;events", 500, 0, 2500);
   ...
   printf("\nmatched events=%ld  12Be-gated (IC %.0f-%.0f)=%ld (%.1f%%)  gated tracks=%ld\n", nAllEvt, icLo, icHi,
   ...
   if (g)
      g->Draw("L same");
   TString png = plotDir + "ic_gated_pid_Be12" + outTag + ".png";
   c->SaveAs(png);
   printf("saved %s\n", png.Data());
}
