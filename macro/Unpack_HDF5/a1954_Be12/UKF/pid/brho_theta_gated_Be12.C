/// @file brho_theta_gated_Be12.C
/// @brief 12Be Bρ vs θ_lab with the IC beam gate (12Be) AND the proton PID gate applied.
...
///
///   root -b -q 'pid/brho_theta_gated_Be12.C("run_0142,run_0143", "/home/yassid/a1954_Be12_reco_hdb_slim/")'
...
void brho_theta_gated_Be12(TString runsCSV = "run_0142", TString inDir = "/home/yassid/a1954_Be12_reco_hdb_slim/",
                           double icLo = 500, double icHi = 900, Int_t icTbLo = 1050, Int_t icTbHi = 1250,
                           double bField = 2.85, double arclenMin = 0, TString outTag = "",
                           TString protonGate =
                              "/home/yassid/fair_install/ATTPCROOTv2-OpenKF/macro/Unpack_HDF5/a1954_Be12/UKF/pid/"
                              "proton_12Be.json")
{
   ...
   TH1D *hIC = new TH1D("hIC", "IC amplitude (red = 12Be gate);ic amplitude;events", 500, 0, 2500);
   TH2F *hPID = new TH2F("hPID", "12Be PID, IC-gated (red = proton gate);#sqrt{dEdx};B#rho [T m]", 400, 0, 45, 400, 0, 1.2);
   TH2F *hBt = new TH2F("hBt", "12Be  B#rho vs #theta_{lab}  (IC + proton gated);#theta_{lab} [deg];B#rho [T m]", 180,
                        0, 180, 300, 0, 1.5);
   ...
   for (int ri = 0; ri < runs->GetEntries(); ++ri) {
      ...
      if (!tF || tF->GetEntries() == 0) { // empty/failed FRIB (e.g. run_0161 unpack crashed)
   ...
   printf("\n== gated Brho-theta ==  matched=%ld  IC-12Be=%ld (%.1f%%)  IC-gated tracks=%ld  +proton=%ld (%.1f%%)\n",
   ...
   TString png = plotDir + "brho_theta_gated_Be12" + outTag + ".png";
   c->SaveAs(png);
   printf("saved %s\n", png.Data());
}
