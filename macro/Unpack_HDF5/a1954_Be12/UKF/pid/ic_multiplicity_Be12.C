...
///
/// Panels: (1) #IC-peaks distribution (events whose max is in the 12Be window)
///         (2) Bp-vs-theta, mult==1 AND single peak in [icLo,icHi]  (CLEAN 12Be)
...
///
///   root -b -q 'pid/ic_multiplicity_Be12.C("run_0142,run_0143,run_0147,run_0148,run_0150,run_0200")'
...
void ic_multiplicity_Be12(TString runsCSV, TString inDir = "/home/yassid/a1954_Be12_reco_hdb_slim/", double icLo = 500,
                          ...
                          TString protonGate = "/home/yassid/fair_install/ATTPCROOTv2-OpenKF/macro/Unpack_HDF5/"
                                               "a1954_Be12/UKF/pid/proton_12Be.json")
{
   ...
   TString plotDir = TString(getenv("VMCWORKDIR")) + "/macro/Unpack_HDF5/a1954_Be12/UKF/pid/plots/";
   AtTools::AtSpyralPID spy; spy.SetBField(bField);
   TCutG *pcut = LoadCut(protonGate.Data(), "pcut");

   TH1D *hN = new TH1D("hN", "IC pulse multiplicity (max in 12Be window);# IC pulses;events", 8, 0.5, 8.5);
   TH2F *b1 = new TH2F("b1", "B#rho vs #theta  MULT==1 (clean 12Be);#theta_{lab} [deg];B#rho [T m]", 180, 0, 180, 300, 0, 1.5);
   ...
   c->cd(3); b2->Draw("colz");
   c->cd(4); b0->Draw("colz");
   TString png=plotDir+"ic_multiplicity_Be12.png"; c->SaveAs(png); printf("saved %s\n",png.Data());
}
