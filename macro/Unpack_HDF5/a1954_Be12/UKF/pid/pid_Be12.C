/// @file pid_Be12.C
/// @brief PID plane (sqrt(dEdx) vs Brho) for a1954 12Be(p,p') from the faithful Spyral
...
///
///   root -b -q 'pid/pid_Be12.C("run_0142")'
///   root -b -q 'pid/pid_Be12.C("run_0142,run_0143,run_0147", "/home/yassid/a1954_Be12_reco/")'
...
void pid_Be12(TString runsCSV = "run_0142", TString inDir = "/home/yassid/a1954_Be12_reco/", Long64_t nEvents = -1,
              ...
              double sqrtMax = 80, double brhoMax = 2.5, double dedxMax = 4000)
{
   ...
   TH2F *hS = new TH2F("hS", "12Be Spyral PID (red = 16C+p proton band);#sqrt{dEdx};B#rho [T m]", 400, 0, sqrtMax, 400,
                       0, brhoMax);
   TH2F *hSd = new TH2F("hSd", "12Be Spyral PID;dEdx [counts];B#rho [T m]", 400, 0, dedxMax, 400, 0, brhoMax);
   TH2F *hKt = new TH2F("hKt", "dEdx vs #theta_{lab};#theta_{lab} [deg];dEdx [counts]", 180, 0, 180, 400, 0, dedxMax);

   TFile *fo = new TFile((plotDir + "pid_Be12" + outTag + ".root").Data(), "RECREATE");
   ...
   c->cd(3);
   hKt->Draw("colz");
   TString png = plotDir + "pid_Be12" + outTag + ".png";
   c->SaveAs(png);
   printf("saved %s\n", png.Data());
   fo->Close();
}
