/// @file fitUKF_Be12.C
/// @brief Run ONLY the new UKF fitter on a pre-reconstructed a1954 12Be(p,p') file.
///
/// Ported from a1975 UKF/pipeline/fitUKF_a1975.C. Reads <fileName>_reco.root
/// (AtPatternEvent, from unpackReco_Be12.C) and runs EventFit::AtFitterUKF wrapped
                                                      ...
                                                      double refInflation = 4.0)
{
   ...
   if (it == kParticleTable.end()) {
      std::cout << "\033[1;31m[fitUKF_Be12] Unknown particle '" << name << "' — skipping.\033[0m" << std::endl;
      return nullptr;
   }
                 ...
                 Double_t refInflation = 4.0)
{
   ...
   if (gSystem->AccessPathName(inputFile.Data())) {
      std::cout << "\033[1;31mERROR: " << inputFile << " not found. Run unpackReco_Be12.C first.\033[0m" << std::endl;
      return;
   }

   std::cout << "\033[1;33m=== fitUKF_Be12 ===\033[0m" << std::endl;
