...
one(){ r="$1"
  ...
  root -b -q -l "$HERE/pipeline/fitUKF_Be12.C(\"$r\",-1,\"proton\",-1,2.85,3.308e-5,\"_s4\",\"$IN\",4.0,0.1,1,10,\"$OUT\")" > "$OUT/logs/${r}_u4.log" 2>&1
  root -b -q -l "$HERE/pipeline/fitGenfit_Be12.C(\"$r\",-1,\"$IN\",\"_s05\",\"$OUT\",-2.85,2,5,\"\",0.5,10.0,170.0,kTRUE,kFALSE,\"proton\")" > "$OUT/logs/${r}_g05.log" 2>&1
