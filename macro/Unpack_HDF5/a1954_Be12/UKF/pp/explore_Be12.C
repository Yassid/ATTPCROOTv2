/// @file explore_Be12.C
/// @brief Interactive explorer for the a1954 12Be(p,p') UKF protons. Loads the cached
/// kinematics ntuple written by pp/ex_Be12.C (e.g. plots/proton_kin_clean155.root) into
...
/// two-body kinematics (same expression as ex_Be12.C): returns {Ex, theta_cm[deg]}
...
/// ejectile KE vs lab angle locus for p(12Be,12Be*)p at excitation Ex (CM-angle scan)
...
class Be12Explorer : public TObject {
public:
   Be12Explorer(TString cache, double mEjectAmu, double mResidAmu, TString tag, double Ebeam0)
   {
      ...
      f->Close();
      std::cout << "Be12Explorer: loaded " << fKe.size() << " ejectiles from " << cache << "\n";
      MakeGui(Ebeam0);
      Redraw();
   }
   ...
   void Redraw()
   {
      ...
      if (fKinLines->IsDown()) { // reference loci at the current Ebeam
         ...
         for (L l : {L{0, kRed + 1, 1}, L{2.10, kGreen + 2, 2}, L{4, kGray + 2, 2}, L{6, kGray + 2, 2}}) {
         ...
         tx->DrawLatex(0.45, 0.81, "E_{x}=2.10 (2^{+}_{1})");
         tx->SetTextColor(kGray + 2);
         tx->DrawLatex(0.45, 0.76, "E_{x}=4, 6 MeV");
      }
      fCanvas->cd(3);
      gPad->SetLogz(1);
      hEt->Draw("colz");
...
private:
   ...
   TGNumberEntry *mkNum(TGCompositeFrame *bar, const char *lab, double val, double lo, double hi, int dig = 0)
   {
      ...
      ne->Connect("ValueSet(Long_t)", "Be12Explorer", this, "Redraw()"); // arrows redraw immediately
   ...
   ClassDef(Be12Explorer, 0);
};

/// defaults = 12Be(p,p') on the clean 73-run cache at the currently calibrated 155 MeV
