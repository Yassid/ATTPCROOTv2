...
one(){ local r="$1"; local L="$LOG/${r}_genfit.log"
  ...
  root -b -q -l "$HERE/pipeline/fitGenfit_Be12.C(\"$r\",-1,\"$IN\",\"\",\"$FITDIR\",-2.85,2,5,\"\",4.0,5.0,170.0,kFALSE,kFALSE,\"deuteron\")" > "$L" 2>&1
  echo "[$(date +%H:%M:%S)] $r genfit=$([ -f $FITDIR${r}_genfit.root ]&&echo ok||echo FAIL)"
}
export -f one; export HERE FITDIR IN LOG
