...
# writes geometry/ATTPC_H600torr.root and ATTPC_H600torr_geomanager.root (6.616e-5 g/cm3)
\end{lstlisting}
...
\code{GasPressure}   & 600 torr & H$_2$, 293~K, 6.616e-5 g/cm$^3$ \\
...
\begin{lstlisting}
root -b -q -l "$HERE/pipeline/fitUKF_Be12.C(\"run_0147\",-1,\"proton\",-1,2.85,6.616e-5,\
  \"\",\"/home/yassid/a1954_Be12_fit_600torr/in/\",0.5,0.1,1,10,\"/home/yassid/a1954_Be12_fit_600torr/\")"
\end{lstlisting}
...
6  & \code{gasDensity}    & 3.308e-5 & g/cm$^3$, feeds CATIMA. \textbf{Pass 6.616e-5} (H$_2$ 600 torr); the default is the wrong 300 torr \\
...
13 & \code{matEffects}     & \code{kTRUE} & keep it --- see below \\
...
16 & \code{geoName}        & \code{ATTPC_H300torr_RT} & must match the real gas density: \textbf{pass \code{ATTPC_H600torr}}; the default is the wrong 300 torr \\
\bottomrule
\end{longtable}
...
\code{gasDensity}     & 6.616e-5  & CATIMA density, H$_2$ at 600 torr / 293~K. Matches \code{media.geo} \code{H\_600torr} exactly. \textbf{The 2026-08-25 rebuild used 3.308e-5 (300 torr), half the material; it put the elastic peak at $+0.2$--$0.3$~MeV. Superseded 2026-09-25.} \\
...
\item \textbf{Beam energy is not calibrated.} 155~MeV comes from zeroing the elastic peak,
      ...
      logbook value before quoting numbers. At the correct 600~torr the elastic peak sits at
      $+0.003$ (UKF) / $-0.010$ (GENFIT)~MeV with 155~MeV --- consistent, but in-sample:
      155 was scanned against 6.5e-5~g/cm$^3$, which is essentially this gas.
\item \textbf{The gas is 600~torr, not 300.} A 2026-08-25 rebuild assumed 300~torr; it was
      wrong and is superseded (2026-09-25). The macro defaults (\code{gasDensity},
      \code{geoName}) still carry the 300-torr values, so the drivers
      (\file{fitpipe_*Be12.sh}) pass \code{GASDENS=6.616e-5} and
      \code{GEONAME=ATTPC_H600torr} explicitly. The a1954 $^{14}$C setting \emph{was} 300~torr.
\item \textbf{Beam energy loss along $v_z$.} The (p,p$'$) elastic peak rises with vertex
      $z$ by 0.19 (UKF) / 0.16 (GENFIT) MeV/m: the beam enters at low $v_z$. CATIMA gives
      9.1~MeV/m for $^{12}$Be at 155~MeV in 600-torr H$_2$, which removes only $\sim 60\,\%$
      of that drift; the explorer's $E_{\rm beam}(v_z)$ control defaults to 9.1 (off).
