void MCanalysis()
{
   ...
   TH2D *ScatterEvsEloss = new TH2D("ScatterEvsEloss", "ScatterEvsEloss", 1000, 0, 0.02, 1000, 0, 10);

   // AtTpcPoint* point = new AtTpcPoint();
   TClonesArray *pointArray = 0;

   TFile *file = new TFile("notpcsim.root", "READ");
   ...
   for (Int_t iEvent = 0; iEvent < nEvents; iEvent++) {
      ...
      for (Int_t i = 0; i < n; i++) {
         ...
         if (VolName == "target_drift_volume") { // 22Mg in the beam pipe
            ...
            if (scatterEloss > 0)
               std::cout << "Event ID : " << iEvent << " Volume Name : " << VolName << " - Track ID : " << trackID
                         << " - Energy Loss : " << scatterEloss << "\n";
