...
///
///   root -b -q 'acceptance_C16pd.C("./diagnostics/acc/gs_sim.root","./diagnostics/acc/gs_ukf.root","gs",0.0)'
///   root -b -q 'acceptance_C16pd.C("./diagnostics/acc/ex1_sim.root","./diagnostics/acc/ex1_ukf.root","ex1",6.094)'
...
void acceptance_C16pd(TString simFile, TString fitFile, TString tag = "gs", Double_t resEx = 0.0, Double_t Ebeam = 192.0,
                    ...
                    Double_t zMin = -1e9, Double_t zMax = 1e9)
{
   ...
   // 16C(p,d)15C: the DETECTED particle is the deuteron, the residual is 15C.
   const double m_C16 = 16.0147 * u, m_p = 1.007825 * u;
   const double m_d = 2.0141017781 * u, m_C15 = 15.0105993 * u;
   const double m_resid = m_C15 + resEx; // the residual is left excited
   ...
   for (Long64_t i = 0; i < N; ++i) {
      ...
      for (int k = 0; k < mc->GetEntriesFast(); ++k) {
         ...
         if (!t || t->GetPdgCode() != 1000010020 || t->GetMotherId() != -1) // deuteron
            continue;
      ...
      if (zT < zMin || zT > zMax)
         continue;
      auto [exT, cmT] = acc_kine(m_C16, m_p, m_d, m_resid, Ebeam, thT, keT);
      if (std::isnan(cmT))
         continue;
      ++nGen;
      hGen->Fill(cmT);
   ...
   hAcc->SetTitle(TString::Format("16C(p,d) acceptance, Ex = %.2f MeV;#theta_{cm} [deg];acceptance", resEx));
   hAcc->SetMinimum(0);
