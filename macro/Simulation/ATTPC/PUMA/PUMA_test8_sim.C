/// @file PUMA_test8_sim.C
/// @brief ATTPCROOT Geant4 transport for PUMA test channel "branch 8":
///        a single pi+/pi- pair emitted back-to-back, each with fixed total
...
/// Uses AtPUMAGenerator with SetChannel(8).
///
/// Run: root -b -q 'PUMA_test8_sim.C(1000)'

void PUMA_test8_sim(Int_t nEvents = 1000, Double_t testEnergy = 0.4, TString mcEngine = "TGeant4")
{
   ...
   // ---- PUMA generator, branch-8 test channel --------------------------
   ...
   if (auto *stack = dynamic_cast<AtStack *>(gMC->GetStack())) {
      ...
      std::cout << "[PUMA_test8_sim] AtStack tuned: StoreSecondaries=false, EnergyCut=1 MeV\n";
   } else {
      std::cout << "[PUMA_test8_sim] WARNING: could not access AtStack to tune cuts.\n";
   }
   if (gMC) {
   ...
   timer.Stop();
   std::cout << "\nPUMA test-8 sim done (E=" << testEnergy << " GeV/particle).\n  Output: " << outFile << "\n  Real "
