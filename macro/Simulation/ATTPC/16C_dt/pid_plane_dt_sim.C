/// @file pid_plane_dt_sim.C
/// @brief The Spyral PID plane (sqrt(dE/dx), Brho) for the SIMULATED 16C(d,t)15C, computed from
///        PATTERN TRACKS exactly as pid_plane_dt.C does for the data. NO FIT REQUIRED.
///
...
/// THE TWO PLANES. So: draw here, on these points, and do not reuse triton_d2_dv1104.json.
///
...
///
/// The five states are kept separate in `state` because their kinematics differ: at Ex = 0 the
/// tritons already sit at the 56 deg lab limit, so the higher states are not a small correction.
/// A gate that suits the g.s. and clips the 6.358 would bias the very ratio being measured.
///
///   root -b -q 'pid_plane_dt_sim.C()'                 // all five states
///   root -b -q 'pid_plane_dt_sim.C("gs_s3001")'       // one
void pid_plane_dt_sim(TString statesCSV = "gs_s3001,ex1_s3001,ex2_s3001,ex3_s3001,ex4_s3001",
                      TString simDir = "/mnt/f/a1975_C16_dt_sim/",
                      TString outCache = "data/pid_plane_dt_sim.root",
                      TString outPng = "data/pid_plane_dt_sim.png", double bField = 2.85,
                      double sqrtMax = 60, double brhoMax = 2.0, Long64_t maxEvt = -1)
{
   ...
   auto *h = new TH2F("hpid", "a1975 D2 SIMULATION, Spyral PID;#sqrt{dE/dx};B#rho [T m]", 300, 0, sqrtMax,
                      300, 0, brhoMax);
   ...
   // Same branch names as pid_plane_dt.C so the gate drawer and every downstream tool work on
