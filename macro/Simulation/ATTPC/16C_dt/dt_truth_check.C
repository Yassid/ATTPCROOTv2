/// @file dt_truth_check.C
/// @brief Validate the 16C(d,t)15C generation against the analytic locus, BEFORE anything is
...
///
///  1. THE LOCUS. The (d,t) locus is double-valued below the ~56 deg turnover and the experiment
///     measures the LOW-KE branch (the high branch needs an ~840 mm radius in a ~290 mm chamber).
///     Analytic low branch at 184.17 MeV: 0.82 MeV at 10 deg, 1.14 at 30 deg, 2.86 at 50 deg.
///     The generator must reproduce those.
///
...
///
///   root -b -q 'dt_truth_check.C("./data/dt_test.root",0.0)'

#include <tuple>

static double dtc_omega2(double x, double y, double z)
{
   return std::sqrt(x * x + y * y + z * z - 2 * x * y - 2 * y * z - 2 * x * z);
}

/// two-body kinematics: returns {Ex, theta_cm[deg]} for a measured ejectile (KE, theta_lab)
static std::tuple<double, double> dtc_kine(double m1, double m2, double m3, double m4, double K_proj, double thetalab,
                                           double K_eject)
{
   double Et1 = K_proj + m1, Et3 = K_eject + m3, Et4 = Et1 + m2 - Et3;
   double s = m1 * m1 + m2 * m2 + 2 * m2 * Et1;
   double u = m2 * m2 + m3 * m3 - 2 * m2 * Et3;
   double m4_ex = std::sqrt((std::cos(thetalab) * dtc_omega2(s, m1 * m1, m2 * m2) * dtc_omega2(u, m2 * m2, m3 * m3) -
   ...
   double theta_cm = TMath::Pi() - std::acos((s * s + s * (2 * t - m1 * m1 - m2 * m2 - m3 * m3 - m4_ex * m4_ex) +
                                             ...
                                             (dtc_omega2(s, m1 * m1, m2 * m2) * dtc_omega2(s, m3 * m3, m4_ex * m4_ex)));
   return {Ex, theta_cm * TMath::RadToDeg()};
}

void dt_truth_check(TString simFile = "./data/dt_test.root", Double_t resEx = 0.0, Double_t Ebeam = 184.17)
{
   gSystem->Load("libAtSimulationData.so");
   gStyle->SetOptStat(0);

   const double u = 931.49401;
   const double m_C16 = 16.0147 * u, m_d = 2.0141017781 * u;
   const double m_t = 3.0160492779 * u, m_C15 = 15.0105993 * u;
   const double m_resid = m_C15 + resEx;
   ...
   auto *hKEth = new TH2D("hKEth", "truth locus;#theta_{lab} [deg];KE_{t} [MeV]", 90, 0, 90, 100, 0, 20);
   ...
   for (Long64_t i = 0; i < N; ++i) {
      ...
      auto [ex, cm] = dtc_kine(m_C16, m_d, m_t, m_resid, Ebeam, th, ke);
   ...
   const double expTh[3] = {10, 30, 50}, expKE[3] = {0.82, 1.14, 2.86};
   for (int b = 0; b < NL; ++b) {
   ...
   c1->SaveAs("./data/dt_truth_check.png");
   printf("\n  wrote data/dt_truth_check.png\n\n");
}
