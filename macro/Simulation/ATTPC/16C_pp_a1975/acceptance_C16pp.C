...
///
///   root -b -q 'acceptance_C16pp.C("./diagnostics/acc/gs_sim.root","./diagnostics/acc/gs_ukf.root","gs",0.0)'
///   root -b -q 'acceptance_C16pp.C("./diagnostics/acc/ex1_sim.root","./diagnostics/acc/ex1_ukf.root","ex1",6.094)'
...
void acceptance_C16pp(TString simFile, TString fitFile, TString tag = "gs", Double_t resEx = 0.0, Double_t Ebeam = 188.0,
                    ...
                    Double_t zMin = -1e9, Double_t zMax = 1e9)
{
   ...
   // 16C(p,p')16C: the DETECTED particle is the proton, the residual is 16C itself.
   const double m_C16 = 16.0147 * u, m_p = 1.007825 * u;
   const double m_resid = m_C16 + resEx; // the residual is left excited
   ...
   for (Long64_t i = 0; i < N; ++i) {
      ...
      for (int k = 0; k < mc->GetEntriesFast(); ++k) {
         ...
         if (!t || t->GetPdgCode() != 2212 || t->GetMotherId() != -1) // proton
            continue;
      ...
      if (zT < zMin || zT > zMax)
         continue;
      auto [exT, cmT] = acc_kine(m_C16, m_p, m_p, m_resid, Ebeam, thT, keT);
      if (std::isnan(cmT))
         continue;
      ++nGen;
      hGen->Fill(cmT);
   ...
   hAcc->SetTitle(TString::Format("16C(p,p') acceptance, Ex = %.2f MeV;#theta_{cm} [deg];acceptance", resEx));
   hAcc->SetMinimum(0);
