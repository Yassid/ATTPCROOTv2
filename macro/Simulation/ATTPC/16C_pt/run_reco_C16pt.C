/// @file run_reco_C16pt.C
/// @brief Digitise + reconstruct the a1975 16C(p,t)15C simulation with the SAME chain the data
/// uses, so the acceptance it yields describes the data production rather than a different one.
///
/// WHY THIS EXISTS WHEN A (p,t) ACCEPTANCE ALREADY DOES. The acceptance in the analysis repo was
...
/// while the data ran AtPSAMax + TC. This macro closes that gap for (p,t).
///
/// THE PAR IS THE DATA'S OWN, ATTPC.a1975_C16_sim.par, and that is deliberate: drift
...
///     ATTPC.a1975_C16_sim.par declares Gain = 10000; the (p,d) simulation par declares
...
///
///   root -l 'run_reco_C16pt.C("/mnt/f/a1975_C16_dt_sim/gs_s3001_sim.root","/mnt/f/a1975_C16_dt_sim/gs_s3001_reco.root")'

#include "../AtBeamHole.h"

void run_reco_C16pt(TString mcFile = "./data/pt_test.root", TString outputFile = "./data/pt_sim_reco.root",
                    TString paramFile = "ATTPC.a1975_C16_sim_g25000.par", Double_t thr = 20, Int_t hdMcs = 20,
                    Int_t hdMs = 8, Int_t nEvents = 0, Double_t holeR = 20.0, TString joinMethod = "mover")
{
   ...
   std::cout << "\n(p,t) sim reco done -> " << outputFile << "   par = " << paramFile << std::endl;
